...
require_once dirname(__DIR__, 2) . '/src/function/activity_analysis_service.php';
...
try {
    ...
    $force = in_array(strtolower(trim((string) ($_GET['recompute'] ?? ''))), ['1', 'true', 'yes'], true);
    $data = activityAnalysisCompute($pdo, $idUsuario, $id, $force);
    ...
    if (!$e instanceof InvalidArgumentException) error_log('StrideBR activity analysis web API: ' . $e->getMessage());
    echo json_encode(['ok' => false, 'error' => $e instanceof InvalidArgumentException ? $e->getMessage() : 'Não foi possível carregar a análise.'], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
}
