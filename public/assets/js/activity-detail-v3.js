(() => {
    ...
    const sourceLabel = value => ({manual:'Web',gps_web:'GPS Web',stridebr_android:'Mobile',strava:'Strava',import:'Importação',importacao:'Importação',workout_session:'Workout Session',quick_register:'Registro rápido'}[String(value || '').toLowerCase()] || String(value || '').replaceAll('_',' ') || '—')
    ...
    const metricCards = items => {
        const clean = (Array.isArray(items) ? items : []).filter(item => item && String(item.rotulo || '').trim() && String(item.valor || '').trim())
    ...
    const routeHtml = activity => {
        const coordinates = activity?.rota?.geojson?.coordinates
        if (!Array.isArray(coordinates) || coordinates.length < 2) return ''
        const gain = Number(activity.rota.ganho_m)
        return `<section class="activity-v3-route" data-activity-v3-route><div class="activity-v3-section-head"><div><span>Rota</span><strong>${escapeHtml(distance(activity.rota.distancia_m))}</strong></div>${Number.isFinite(gain) ? `<small>+${escapeHtml(number(gain,0))} m</small>` : ''}</div><div class="activity-v3-map" data-activity-v3-map aria-label="Mapa da rota"></div><small class="activity-v3-map-fallback" data-activity-v3-map-status>Carregando mapa…</small></section>`
    }
    ...
    const detailsHtml = activity => {
        ...
        const data = (Array.isArray(activity.dados) ? activity.dados : []).filter(item => item?.rotulo && item?.valor)
        const units = (Array.isArray(activity.unidades) ? activity.unidades : []).filter(unit => Array.isArray(unit.valores) && unit.valores.length)
        return `${rows.length ? `<section class="activity-v3-section"><div class="activity-v3-info-list">${rows.map(([label,value]) => `<div><span>${escapeHtml(label)}</span><strong>${escapeHtml(value)}</strong></div>`).join('')}</div></section>` : ''}${data.length ? `<section class="activity-v3-section"><div class="activity-v3-section-head"><div><span>Dados</span></div></div>${metricCards(data)}</section>` : ''}${units.length ? `<section class="activity-v3-section"><div class="activity-v3-section-head"><div><span>Trechos registrados</span></div></div><div class="activity-v3-unit-list">${units.map(unit => `<article><strong>${escapeHtml(unit.rotulo || 'Trecho')}</strong>${unit.modalidade ? `<small>${escapeHtml(unit.modalidade)}</small>` : ''}${metricCards(unit.valores)}</article>`).join('')}</div></section>` : ''}${activity.observacoes ? `<section class="activity-v3-section"><div class="activity-v3-section-head"><div><span>Observações</span></div></div><p class="activity-v3-notes">${escapeHtml(activity.observacoes).replace(/\n/g,'<br>')}</p></section>` : ''}`
    }
    const tabsFor = activity => {
        const caps = activity?.stream_capabilities || {}
        const tabs = [{id:'summary',label:'Resumo'}]
        if (caps.has_streams) tabs.push({id:'charts',label:'Gráficos'})
        if (caps.has_streams && (caps.available_streams || []).includes('distance')) tabs.push({id:'splits',label:'Splits'})
        if (caps.has_laps) tabs.push({id:'laps',label:'Voltas'})
        if (caps.has_streams && (caps.has_heart_rate || String(activity.metrica_derivada || '') === 'pace_km')) tabs.push({id:'zones',label:'Zonas'})
        if (caps.has_streams) tabs.push({id:'analysis',label:'Análise'})
        return tabs
    }
    const shellHtml = activity => {
        const tabs = tabsFor(activity)
        const summary = `${metricCards(activity.metricas || [])}${routeHtml(activity)}${strengthHtml(activity)}${detailsHtml(activity)}`
        const routeAction=activity?.rota?.geojson?.coordinates?.length>1?`<button type="button" class="activity-secondary-button" data-save-activity-route="${escapeHtml(activity.id)}">Salvar rota</button>`:''
        return `<div class="activity-v3" data-activity-v3><nav class="activity-v3-tabs" aria-label="Detalhes da atividade">${tabs.map((tab,index) => `<button type="button" data-activity-v3-tab="${tab.id}" aria-selected="${index===0?'true':'false'}">${escapeHtml(tab.label)}</button>`).join('')}</nav><div class="activity-v3-panels">${tabs.map((tab,index) => `<section class="activity-v3-panel" data-activity-v3-panel="${tab.id}"${index===0?'':' hidden'}>${tab.id==='summary' ? summary : `<div class="activity-v3-lazy" data-activity-v3-lazy="${tab.id}"><span></span><strong>Carregando ${escapeHtml(tab.label.toLowerCase())}…</strong></div>`}</section>`).join('')}</div><footer class="activity-detail-actions has-share"><button type="button" class="activity-secondary-button" data-share-activity>Compartilhar</button>${routeAction}<button type="button" class="activity-secondary-button" data-stats-activity="${escapeHtml(activity.id)}" aria-pressed="${activity.excluded_from_stats?'true':'false'}">${activity.excluded_from_stats?'Incluir nas estatísticas':'Excluir das estatísticas'}</button><a class="activity-secondary-button" data-repeat-activity href="/user/atividades.php?repetir=${encodeURIComponent(activity.id)}">Repetir</a><a class="activity-primary-action" href="/user/editatividade.php?id=${encodeURIComponent(activity.id)}">Editar</a><button type="button" class="activity-danger-button" data-delete-activity="${escapeHtml(activity.id)}" data-activity-title="${escapeHtml(activity.titulo)}">Excluir</button></footer></div>`
    }
    ...
    const metricDefinitions = data => {
        ...
        if (available.has('grade')) definitions.push({id:'grade',label:'Inclinação',unit:'percent'})
    ...
    const metricValue = (sample, metric) => {
        if (metric === 'pace') return sample.pace
        if (metric === 'speed') return Number.isFinite(Number(sample.speed)) ? Number(sample.speed) * 3.6 : null
    ...
    const metricFormat = (value, definition) => {
        ...
        if (definition.id === 'grade') return percent(value)
    ...
    const chartsMarkup = data => {
        ...
        const charts = definitions.filter(def => enabled.has(def.id)).map(def => chartHtml(def,data.samples || [])).join('')
        return `<div class="activity-v3-chart-tools"><div class="activity-v3-axis"><button type="button" data-chart-axis="time" class="${data.axis==='time'?'is-active':''}">Tempo</button><button type="button" data-chart-axis="distance" class="${data.axis==='distance'?'is-active':''}">Distância</button></div><div class="activity-v3-chart-toggles">${toggles}</div></div><div class="activity-v3-chart-tooltip" data-chart-tooltip><strong>Mova o cursor sobre o gráfico</strong><span>As séries usam o mesmo eixo.</span></div><div class="activity-v3-chart-list" data-chart-list>${charts || '<div class="activity-v3-empty">Nenhuma série disponível.</div>'}</div>`
    }
    ...
    const bindCharts = (state, panel, data) => {
        ...
        const rerender=()=>{
            ...
            if(list) list.innerHTML=definitions.filter(def=>active.has(def.id)).map(def=>chartHtml(def,data.samples||[])).join('') || '<div class="activity-v3-empty">Selecione uma métrica.</div>'
            bindSurfaces()
        }
        ...
        panel.querySelectorAll('[data-chart-axis]').forEach(button=>button.addEventListener('click',async()=>{
            ...
            if(axis===data.axis)return
            panel.innerHTML='<div class="activity-v3-lazy"><span></span><strong>Atualizando eixo…</strong></div>'
            await loadCharts(state,panel,axis)
        }))
        bindSurfaces()
    ...
    const loadCharts = async (state,panel,axis='distance') => {
        try {
            const streams=(state.activity.stream_capabilities?.available_streams||[]).filter(value=>['pace','speed','heart_rate','altitude','elevation','grade','cadence','power','temperature'].includes(value)).join(',')
            let resolvedAxis=axis
            if(axis==='distance' && !(state.activity.stream_capabilities?.available_streams||[]).includes('distance')) resolvedAxis='time'
            const data=await fetchJson(`/api/atividade-streams.php?id=${encodeURIComponent(state.activity.id)}&axis=${resolvedAxis}&resolution=medium&streams=${encodeURIComponent(streams)}`)
            panel.innerHTML=chartsMarkup(data)
            bindCharts(state,panel,data)
        } catch(error) {
            panel.innerHTML=`<div class="activity-v3-error"><strong>Não foi possível carregar os gráficos.</strong><span>${escapeHtml(error.message)}</span></div>`
        }
    }
    const splitTable = data => {
        ...
        return `<div class="activity-v3-table-wrap"><table class="activity-v3-table"><thead><tr><th>Split</th><th>Distância</th><th>Tempo</th><th>${performanceLabel}</th><th>FC méd.</th><th>FC máx.</th><th>Elevação</th><th>Cadência</th></tr></thead><tbody>${rows.map(row=>`<tr><th>${escapeHtml(row.index)}${row.partial?'*':''}</th><td>${escapeHtml(distance(row.distance_m))}</td><td>${escapeHtml(duration(row.moving_duration_s ?? row.elapsed_duration_s))}</td><td>${escapeHtml(performance(row))}</td><td>${row.heart_rate_avg_bpm!==null?`${escapeHtml(number(row.heart_rate_avg_bpm,0))} bpm`:'—'}</td><td>${row.heart_rate_max_bpm!==null?`${escapeHtml(number(row.heart_rate_max_bpm,0))} bpm`:'—'}</td><td>${row.elevation_gain_m!==null?`+${escapeHtml(number(row.elevation_gain_m,0))} m`:'—'}</td><td>${row.cadence_avg!==null?escapeHtml(number(row.cadence_avg,0)):'—'}</td></tr>`).join('')}</tbody></table></div>${rows.some(row=>row.partial)?'<small class="activity-v3-table-note">* Split parcial no final da atividade.</small>':''}`
    }
    const loadSplits = async (state,panel,distanceM=1000) => {
        ...
        try {
            const data=await fetchJson(`/api/atividade-splits.php?id=${encodeURIComponent(state.activity.id)}&distance_m=${encodeURIComponent(distanceM)}`)
            ...
            panel.innerHTML=`<div class="activity-v3-error"><strong>Não foi possível carregar os splits.</strong><span>${escapeHtml(error.message)}</span></div>`
        }
    }
    const lapsTable = data => {
        ...
        const performanceLabel=speedMode?'Velocidade':'Pace'
        const performance=row=>speedMode?(row.speed_kmh!==null?speed(row.speed_kmh):'—'):(row.pace_s_per_km!==null?pace(row.pace_s_per_km):'—')
        return `<div class="activity-v3-table-wrap"><table class="activity-v3-table"><thead><tr><th>Volta</th><th>Distância</th><th>Tempo</th><th>${performanceLabel}</th><th>FC méd.</th><th>Elevação</th><th>Origem</th></tr></thead><tbody>${rows.map(row=>`<tr><th>${escapeHtml(row.order)}</th><td>${escapeHtml(distance(row.distance_m))}</td><td>${escapeHtml(duration(row.moving_duration_s ?? row.elapsed_duration_s))}</td><td>${escapeHtml(performance(row))}</td><td>${row.heart_rate_avg_bpm!==null?`${escapeHtml(number(row.heart_rate_avg_bpm,0))} bpm`:'—'}</td><td>${row.elevation_gain_m!==null?`+${escapeHtml(number(row.elevation_gain_m,0))} m`:'—'}</td><td>${escapeHtml(row.origin==='manual'?'Manual':row.origin==='import'?'Importada':row.origin)}</td></tr>`).join('')}</tbody></table></div>`
    }
    const loadLaps = async (state,panel) => {
        try {
            const data=await fetchJson(`/api/atividade-laps.php?id=${encodeURIComponent(state.activity.id)}`)
            panel.innerHTML=lapsTable(data)
        } catch(error) {
            panel.innerHTML=`<div class="activity-v3-error"><strong>Não foi possível carregar as voltas.</strong><span>${escapeHtml(error.message)}</span></div>`
        }
    }
    ...
    const getAnalysis = state => {
        if(!state.analysisPromise) state.analysisPromise=fetchJson(`/api/atividade-analysis.php?id=${encodeURIComponent(state.activity.id)}`)
        return state.analysisPromise
    }
    const getPlannedActual = state => {
        if(!state.plannedActualPromise) state.plannedActualPromise=fetchJson(`/api/atividade-planned-actual.php?id=${encodeURIComponent(state.activity.id)}`)
    ...
    const loadZones = async (state,panel) => {
        try {
            ...
            panel.innerHTML=`<div class="activity-v3-error"><strong>Não foi possível carregar as zonas.</strong><span>${escapeHtml(error.message)}</span></div>`
        }
    }
    ...
    const analysisHtml = analysis => {
        ...
        if(elevation){metrics.push(['Ganho',`+${number(elevation.gain_m,0)} m`],['Perda',`-${number(elevation.loss_m,0)} m`])}
    ...
    const loadAnalysis = async (state,panel) => {
        ...
        if(analysisResult.status==='rejected') {
            panel.innerHTML=`<div class="activity-v3-error"><strong>Não foi possível carregar a análise.</strong><span>${escapeHtml(analysisResult.reason?.message||'Tente novamente.')}</span></div>`
            return
        }
        panel.innerHTML=analysisHtml(analysisResult.value)+(plannedResult.status==='fulfilled'?plannedActualHtml(plannedResult.value):'')
    }
    const activateTab = async (state,id) => {
        state.root.querySelectorAll('[data-activity-v3-tab]').forEach(button=>button.setAttribute('aria-selected',button.dataset.activityV3Tab===id?'true':'false'))
        ...
        if(!panel || state.loaded.has(id))return
        state.loaded.add(id)
        if(id==='charts')await loadCharts(state,panel,'distance')
        if(id==='splits')await loadSplits(state,panel,1000)
        if(id==='laps')await loadLaps(state,panel)
        if(id==='zones')await loadZones(state,panel)
        if(id==='analysis')await loadAnalysis(state,panel)
    }
    const mountMap = async state => {
        ...
        const coordinates=state.activity?.rota?.geojson?.coordinates
        if(!element || !Array.isArray(coordinates)||coordinates.length<2)return
        try{
            state.mapController=await window.StrideBRWebMap?.mount?.(element,coordinates,{controls:true,maxZoom:17})
            if(status)status.hidden=true
        }catch(error){
            if(status){status.hidden=false;status.textContent='Mapa indisponível. A rota continua disponível nos dados da atividade.'}
        }
    }
    ...
    const render = ({content,activity}) => {
        if(!content||!activity)return false
        destroy(content)
        content.innerHTML=shellHtml(activity)
        bindStatsToggle(content,activity)
        bindRouteSave(content,activity)
        content.hidden=false
        const root=content.querySelector('[data-activity-v3]')
        const state={root,activity,loaded:new Set(['summary']),analysisPromise:null,plannedActualPromise:null,mapController:null,streamData:null}
        states.set(content,state)
        root.querySelectorAll('[data-activity-v3-tab]').forEach(button=>button.addEventListener('click',()=>activateTab(state,button.dataset.activityV3Tab)))
