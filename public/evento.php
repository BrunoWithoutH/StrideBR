...
$program = array_values(array_filter((array) ($event['provas'] ?? []), 'is_array'));
$programGroups = [];
foreach ($program as $item) {
    $group = trim((string) ($item['grupo'] ?? '')) ?: 'other';
    $programGroups[$group][] = $item;
}
$sports = array_values(array_filter((array) ($event['modalidades'] ?? []), 'is_array'));
$externalSources = array_values(array_filter((array) ($event['fontes_externas'] ?? []), 'is_array'));
$externalUrls = array_fill_keys(array_values(array_filter(array_map(static fn(array $source): string => trim((string) ($source['source_url'] ?? '')), $externalSources))), true);
$manualSources = array_values(array_filter((array) ($event['fontes'] ?? []), static fn(array $source): bool => !isset($externalUrls[trim((string) ($source['url'] ?? ''))])));
$providerLabels = ['faergs' => 'FAERGS', 'cbat' => 'CBAt', 'cbc' => 'CBC', 'fgc' => 'FGC', 'cbtri' => 'CBTri'];
$eventIndexable = !isset($event['seo_indexavel']) || stridebr_db_bool($event['seo_indexavel']);
$timeKnown = !isset($event['horario_informado']) || stridebr_db_bool($event['horario_informado']);
...
<head>
    ...
    <?php echo stridebr_seo_head(['title' => (string) $event['titulo'] . ' — StrideBR', 'description' => $descriptionMeta, 'path' => '/evento.php', 'event_slug' => (string) $event['slug'], 'image' => (string) $mainImage, 'image_alt' => (string) ($images[0]['texto_alternativo'] ?? $event['titulo']), 'locale' => stridebr_locale(), 'type' => 'article', 'noindex' => !$eventIndexable, 'structured' => $eventIndexable ? $structuredEvent : null]); ?>
...
<main class="main-content"><div class="page-shell event-detail-page">
    ...
    <article class="event-detail-hero<?php echo $event['status'] === 'cancelado' ? ' is-cancelled' : ''; ?>">
        ...
        <div class="event-detail-intro">
            ...
            <p class="event-detail-date"><strong><?php echo stridebr_e(stridebr_format_date_short($start)); ?></strong><?php if ($timeKnown): ?> · <?php echo stridebr_e($start->format('H:i')); ?><?php endif; ?><?php if ($end): ?> <?php echo stridebr_e(stridebr_t('event.until')); ?> <?php echo stridebr_e($end->format($end->format('Y-m-d') === $start->format('Y-m-d') ? ($timeKnown ? 'H:i' : (stridebr_locale() === 'en' ? 'M j, Y' : 'd/m/Y')) : ($timeKnown ? (stridebr_locale() === 'en' ? 'M j, Y H:i' : 'd/m/Y H:i') : (stridebr_locale() === 'en' ? 'M j, Y' : 'd/m/Y')))); ?><?php endif; ?></p>
    ...
    <div class="event-detail-grid">
        ...
        <aside class="content-card event-facts"><h2><?php echo stridebr_e(stridebr_t('event.information')); ?></h2><dl><?php if ($event['organizador']): ?><div><dt><?php echo stridebr_e(stridebr_t('event.organizer')); ?></dt><dd><?php echo stridebr_e((string) $event['organizador']); ?></dd></div><?php endif; ?><div><dt><?php echo stridebr_e(stridebr_t('common.date')); ?></dt><dd><?php echo stridebr_e($timeKnown ? stridebr_format_datetime_short($start) : stridebr_format_date_short($start)); ?></dd></div><?php if ($location !== ''): ?><div><dt><?php echo stridebr_e(stridebr_t('event.location')); ?></dt><dd><?php echo stridebr_e($location); ?></dd></div><?php endif; ?><?php if ($event['endereco']): ?><div><dt><?php echo stridebr_e(stridebr_t('event.address')); ?></dt><dd><?php echo stridebr_e((string) $event['endereco']); ?></dd></div><?php endif; ?><?php if (count($sports) > 1): ?><div><dt><?php echo stridebr_e(stridebr_t('event.sports')); ?></dt><dd><?php echo stridebr_e(implode(' · ', array_map(static fn(array $sport): string => (string) ($sport['nome'] ?? ''), $sports))); ?></dd></div><?php endif; ?></dl></aside>
    </div>


    <?php if ($programGroups !== []): ?><section class="content-card event-program"><h2><?php echo stridebr_e(stridebr_t('event.program')); ?></h2><div class="event-program-groups"><?php foreach ($programGroups as $group => $items): ?><section class="event-program-group"><h3><?php echo stridebr_e(stridebr_t('event.program_group.' . (in_array($group, ['corrida','barreiras','obstaculos','marcha','revezamento','salto','arremesso_lancamento','combinadas'], true) ? $group : 'other'))); ?></h3><ul><?php foreach ($items as $item): ?><li><strong><?php echo stridebr_e((string) $item['nome']); ?></strong><?php $meta = array_values(array_filter([(string) ($item['categoria'] ?? ''), (string) ($item['sexo'] ?? ''), (string) ($item['fase'] ?? '')], static fn(string $value): bool => trim($value) !== '')); $itemMetadata = $item['metadados'] ?? []; if (!is_array($itemMetadata)) $itemMetadata = json_decode((string) $itemMetadata, true) ?: []; if (!empty($itemMetadata['time'])) $meta[] = (string) $itemMetadata['time']; ?><?php if ($meta !== []): ?><span><?php echo stridebr_e(implode(' · ', $meta)); ?></span><?php endif; ?></li><?php endforeach; ?></ul></section><?php endforeach; ?></div></section><?php endif; ?>
    ...
    <section class="content-card event-sources-public"><h2><?php echo stridebr_e(stridebr_t('event.sources')); ?></h2><p><?php echo stridebr_e(stridebr_t('event.disclaimer')); ?></p><?php if ($externalSources !== [] || $manualSources !== []): ?><ul><?php foreach ($externalSources as $source): ?><?php $provider = strtolower(trim((string) ($source['provider'] ?? ''))); $providerLabel = $providerLabels[$provider] ?? strtoupper($provider); ?><li><span><?php echo stridebr_e(stridebr_t('event.source_label', ['source' => $providerLabel])); ?></span> · <a href="<?php echo stridebr_e((string) $source['source_url']); ?>" target="_blank" rel="noopener noreferrer"><?php echo stridebr_e(stridebr_t('event.view_source')); ?></a></li><?php endforeach; ?><?php foreach ($manualSources as $source): ?><li><a href="<?php echo stridebr_e((string) $source['url']); ?>" target="_blank" rel="noopener noreferrer"><?php echo stridebr_e((string) $source['nome']); ?> ↗</a></li><?php endforeach; ?></ul><?php elseif ($event['url_oficial']): ?><p><a href="<?php echo stridebr_e((string) $event['url_oficial']); ?>" target="_blank" rel="noopener noreferrer"><?php echo stridebr_e(stridebr_t('event.official_site')); ?></a></p><?php else: ?><p><?php echo stridebr_e(stridebr_t('event.no_sources')); ?></p><?php endif; ?></section>
