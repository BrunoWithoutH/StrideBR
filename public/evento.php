...
if ($placeName !== '' || trim((string) ($event['endereco'] ?? '')) !== '') {
    $structuredEvent['location'] = [
        '@type' => 'Place',
        'name' => $placeName !== '' ? $placeName : 'Local do evento',
        'address' => [
            '@type' => 'PostalAddress',
...
<main class="main-content"><div class="page-shell event-detail-page">
    ...
    <a class="event-back-link" href="/calendario.php">← Eventos</a>
    <article class="event-detail-hero<?php echo $event['status'] === 'cancelado' ? ' is-cancelled' : ''; ?>">
        ...
        <div class="event-detail-intro">
            <div class="event-detail-kind"><?php if (!empty($event['modalidade_slug'])): ?><span><?php echo stridebr_sport_icon_html((string) $event['modalidade_slug'], 'sport-icon'); ?></span><?php endif; ?><span><?php echo stridebr_e((string) ($event['tipo'] ?: $event['modalidade_nome'] ?: 'Evento esportivo')); ?></span><?php if ($event['status'] === 'cancelado'): ?><b>Cancelado</b><?php endif; ?></div>
            <h1><?php echo stridebr_e((string) $event['titulo']); ?></h1>
            <p class="event-detail-date"><strong><?php echo stridebr_e($start->format('d/m/Y')); ?></strong> · <?php echo stridebr_e($start->format('H:i')); ?><?php if ($end): ?> até <?php echo stridebr_e($end->format($end->format('Y-m-d') === $start->format('Y-m-d') ? 'H:i' : 'd/m/Y H:i')); ?><?php endif; ?></p>
            ...
            <div class="event-detail-actions">
                <?php if ($event['status'] !== 'cancelado' && $event['url_inscricao']): ?><a class="primary-button" href="<?php echo stridebr_e((string) $event['url_inscricao']); ?>" target="_blank" rel="noopener noreferrer">Abrir inscrição ↗</a><?php endif; ?>
                <?php if ($event['url_oficial']): ?><a class="secondary-button" href="<?php echo stridebr_e((string) $event['url_oficial']); ?>" target="_blank" rel="noopener noreferrer">Site oficial ↗</a><?php endif; ?>
                <?php if ($userId !== null): ?><form method="post"><?php echo stridebr_csrf_field(); ?><button class="secondary-button" type="submit"><?php echo stridebr_db_bool($event['salvo']) ? '★ Evento salvo' : '☆ Salvar evento'; ?></button></form><?php else: ?><a class="secondary-button" href="/login.php">Entre para salvar</a><?php endif; ?>
            </div>
            <?php if ($deadline): ?><p class="event-registration-deadline">Inscrições informadas até <strong><?php echo stridebr_e($deadline->format('d/m/Y H:i')); ?></strong>.</p><?php endif; ?>
        </div>
    </article>

    <div class="event-detail-grid">
        <section class="content-card event-description"><h2>Sobre o evento</h2><?php if ($event['descricao']): ?><p><?php echo nl2br(stridebr_e((string) $event['descricao'])); ?></p><?php else: ?><p>A descrição detalhada ainda não foi cadastrada. Use as fontes oficiais abaixo para confirmar informações antes de se inscrever.</p><?php endif; ?></section>
        <aside class="content-card event-facts"><h2>Informações</h2><dl><?php if ($event['organizador']): ?><div><dt>Organizador</dt><dd><?php echo stridebr_e((string) $event['organizador']); ?></dd></div><?php endif; ?><div><dt>Data</dt><dd><?php echo stridebr_e($start->format('d/m/Y · H:i')); ?></dd></div><?php if ($location !== ''): ?><div><dt>Local</dt><dd><?php echo stridebr_e($location); ?></dd></div><?php endif; ?><?php if ($event['endereco']): ?><div><dt>Endereço</dt><dd><?php echo stridebr_e((string) $event['endereco']); ?></dd></div><?php endif; ?></dl></aside>
    </div>

    <?php if (count($images) > 1): ?><section class="event-gallery-section"><div class="section-title-row"><div><h2>Imagens</h2><p>Fotos e artes cadastradas para este evento.</p></div></div><div class="event-public-gallery"><?php foreach ($images as $image): ?><a href="<?php echo stridebr_e((string) $image['caminho']); ?>" target="_blank" rel="noopener"><img src="<?php echo stridebr_e((string) $image['caminho']); ?>" alt="<?php echo stridebr_e((string) ($image['texto_alternativo'] ?: $event['titulo'])); ?>" loading="lazy"></a><?php endforeach; ?></div></section><?php endif; ?>

    <section class="content-card event-sources-public"><h2>Fontes</h2><p>O StrideBR organiza informações de eventos, mas não é o organizador. Confirme preço, regulamento, percurso, categorias e eventuais mudanças diretamente nas fontes.</p><?php if (!empty($event['fontes'])): ?><ul><?php foreach ($event['fontes'] as $source): ?><li><a href="<?php echo stridebr_e((string) $source['url']); ?>" target="_blank" rel="noopener noreferrer"><?php echo stridebr_e((string) $source['nome']); ?> ↗</a></li><?php endforeach; ?></ul><?php elseif ($event['url_oficial']): ?><p><a href="<?php echo stridebr_e((string) $event['url_oficial']); ?>" target="_blank" rel="noopener noreferrer">Site oficial ↗</a></p><?php else: ?><p>Nenhuma fonte pública foi cadastrada ainda.</p><?php endif; ?></section>
</div></main>
</div>
