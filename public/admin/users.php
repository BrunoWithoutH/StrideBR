...
if($q!==''){
    $needle='%'.$q.'%';
    $sql.=" AND (idusuario::text ILIKE :q_id OR COALESCE(NULLIF(nome_exibicao,''),nomeusuario) ILIKE :q_name OR emailusuario ILIKE :q_email OR username ILIKE :q_username)";
    $params[':q_id']=$needle; $params[':q_name']=$needle; $params[':q_email']=$needle; $params[':q_username']=$needle;
}
...
<!doctype html><html lang="<?php echo function_exists('stridebr_html_lang') ? stridebr_e(stridebr_html_lang()) : 'pt-BR'; ?>"><head>
    <?php if (function_exists('stridebr_ui_boot_script')) echo stridebr_ui_boot_script(); ?><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><link rel="stylesheet" href="<?php echo stridebr_e(stridebr_asset('/assets/css/style.css'));?>"><title>Usuários | StrideBR Admin</title><link rel="stylesheet" href="<?php echo stridebr_e(stridebr_asset('/assets/css/ui-refresh.css')); ?>"><link rel="stylesheet" href="<?php echo stridebr_e(stridebr_asset('/assets/css/admin.css')); ?>">
</head><body class="admin-body"><div class="container-fluid"><?php require dirname(__DIR__,2).'/src/layout/header.php';?><main class="main-content"><div class="admin-shell"><?php echo stridebr_admin_nav('users');?><div class="admin-heading"><div><span class="eyebrow">Administração</span><h1>Usuários</h1><p>Pesquise, edite, bloqueie e gerencie acesso sem tocar direto no banco.</p></div></div><?php foreach($flashes as $f):?><div class="alert alert-<?php echo stridebr_e($f['type']);?>"><?php echo stridebr_e($f['message']);?></div><?php endforeach;?><?php foreach($errors as $e):?><div class="alert alert-danger"><?php echo stridebr_e($e);?></div><?php endforeach;?><section class="admin-card"><form class="admin-filter" method="get"><input name="q" value="<?php echo stridebr_e($q);?>" placeholder="Nome, @username, e-mail ou ID"><select name="status"><option value="">Todos os status</option><option value="Ativo"<?php echo $status==='Ativo'?' selected':'';?>>Ativo</option><option value="Desativado"<?php echo $status==='Desativado'?' selected':'';?>>Bloqueado</option></select><select name="role"><option value="">Todos os papéis</option><?php foreach(['user','moderator','admin','owner'] as $r):?><option value="<?php echo $r;?>"<?php echo $role===$r?' selected':'';?>><?php echo $r;?></option><?php endforeach;?></select><button class="secondary-action">Filtrar</button></form><div class="admin-table-wrap admin-users-table"><table><thead><tr><th>Usuário</th><th>Papel</th><th>Status</th><th>E-mail</th><th>Cadastro</th><th>Último login</th><th></th></tr></thead><tbody><?php foreach($users as $u):?><?php $identityInvalid=!stridebr_person_name_is_valid((string)$u['nome_exibicao'],60)||(!empty($u['username'])&&!stridebr_username_is_valid((string)$u['username']));?><tr><td class="admin-user-cell" data-label="Usuário"><div class="admin-user-identity"><strong><?php echo stridebr_e(stridebr_person_name_for_display((string)$u['nome_exibicao'], (string)($u['username'] ?? ''), 'Usuário', 60));?></strong><small><?php echo $u['username']?'@'.stridebr_e($u['username']):'sem username';?> · ID <?php echo stridebr_e((string)$u['idusuario']);?></small><?php if($identityInvalid):?><span class="identity-warning">identidade fora do padrão</span><?php endif;?></div></td><td data-label="Papel"><?php echo stridebr_e($u['papelusuario']);?></td><td data-label="Status"><span class="status-pill <?php echo $u['statususuario']==='Ativo'?'is-ok':'is-blocked';?>"><?php echo stridebr_e($u['statususuario']);?></span></td><td data-label="E-mail"><?php echo stridebr_e($u['emailusuario']);?><small><?php echo $u['email_verificado_em']||stridebr_db_bool($u['verificado'])?'verificado':'não verificado';?></small></td><td data-label="Cadastro"><?php echo stridebr_e(stridebr_format_datetime_short((string)($u['dataregistrousuario']??'')));?></td><td data-label="Último login"><?php echo !empty($u['ultimologin'])?stridebr_e(stridebr_format_datetime_short((string)$u['ultimologin'])):'—';?></td><td data-label="Ações"><a class="secondary-action compact" href="/admin/user.php?id=<?php echo rawurlencode($u['idusuario']);?>">Gerenciar</a></td></tr><?php endforeach;?></tbody></table></div></section><section class="admin-card admin-table-card"><div class="admin-card-heading"><div><h2>Convites de cadastro</h2><p>Útil se você ativar <code>registration.invite_only.enabled</code>.</p></div></div><form method="post" class="admin-inline-form"><?php echo stridebr_csrf_field();?><input type="hidden" name="action" value="create_invite"><label>Usos<input type="number" name="usos_maximos" min="1" max="50" value="1"></label><label>Validade (dias)<input type="number" name="dias" min="1" max="90" value="14"></label><button class="primary-action">Gerar convite</button></form><div class="admin-table-wrap"><table><thead><tr><th>Prefixo</th><th>Uso</th><th>Expira</th><th>Status</th><th></th></tr></thead><tbody><?php foreach($invites as $i):?><tr><td><code><?php echo stridebr_e($i['prefixo']);?>…</code></td><td><?php echo (int)$i['usos'];?>/<?php echo (int)$i['usos_maximos'];?></td><td><?php echo stridebr_e($i['expira_em']??'sem prazo');?></td><td><?php echo stridebr_db_bool($i['ativo'])?'ativo':'revogado/usado';?></td><td><?php if(stridebr_db_bool($i['ativo'])):?><form method="post"><?php echo stridebr_csrf_field();?><input type="hidden" name="action" value="revoke_invite"><input type="hidden" name="idconvite" value="<?php echo stridebr_e($i['idconvite']);?>"><button class="danger-link">Revogar</button></form><?php endif;?></td></tr><?php endforeach;?></tbody></table></div></section></div></main></div><?php require dirname(__DIR__,2).'/src/layout/footer.php';?></body></html>
