...
$openNotificationId = trim((string) ($_GET['open'] ?? ''));
if ($openNotificationId !== '') {
    $notification = notificacaoBuscar($pdo, $idUsuario, $openNotificationId);
    $destination = $notification !== null ? notificacaoDestinoSeguro((string) ($notification['url'] ?? '')) : '';
    if ($notification !== null && $destination !== '') {
        notificacaoMarcarLida($pdo, $idUsuario, $openNotificationId);
        header('Location: ' . $destination, true, 303);
        exit;
    }
    header('Location: /user/notificacoes.php', true, 303);
    exit;
}
