...
$savedRoutes = routeSavedList($pdo, $idUsuario, false);
$friends = [];
if ($friendsEnabled) {
            ...
            <?php else: ?>
                ...
                <div class="workout-editor-modal" data-workout-editor<?php echo $treinoEdicao === [] ? ' hidden' : ''; ?>>
                    ...
                    <form method="POST" class="workout-form">
                        ...
                        <label class="editor-pacer-field" data-pacer-plan-field><span>Estratégia de pace</span><select name="pacer_plan_id" data-editor-pacer><option value="">Sem Pacer</option><?php foreach ($pacerPlans as $plan): ?><option value="<?php echo stridebr_e((string) $plan['id']); ?>" data-sport="<?php echo stridebr_e((string) $plan['sport']['id']); ?>"<?php echo (string) ($treinoEdicao['idpacerplan'] ?? '') === (string) $plan['id'] ? ' selected' : ''; ?>><?php echo stridebr_e((string) $plan['name']); ?> · <?php echo stridebr_e(number_format((float) $plan['target_distance_m'] / 1000, 2, ',', '.')); ?> km</option><?php endforeach; ?></select><small><a href="/user/pacer.php" target="_blank" rel="noopener">Gerenciar Pacer</a></small></label>
                        <label class="editor-route-field" data-route-field><span>Rota</span><select name="route_id" data-editor-route><option value="">Sem rota</option><?php foreach ($savedRoutes as $route): ?><option value="<?php echo stridebr_e((string) $route['idrota_salva']); ?>" data-sport="<?php echo stridebr_e((string) ($route['idmodalidade'] ?? '')); ?>"<?php echo (string) ($treinoEdicao['idrota_salva'] ?? '') === (string) $route['idrota_salva'] ? ' selected' : ''; ?>><?php echo stridebr_e((string) $route['nome']); ?><?php echo is_numeric($route['distancia_m'] ?? null) ? ' · ' . stridebr_e(number_format((float) $route['distancia_m'] / 1000, 2, ',', '.')) . ' km' : ''; ?></option><?php endforeach; ?></select><small><a href="/user/rotas.php" target="_blank" rel="noopener">Minhas Rotas</a></small></label>
                ...
                <script type="application/json" data-workout-editor-data><?php echo json_encode(array_map(static fn(array $item): array => [
                    ...
                    'pacer_plan_id' => (string) ($item['idpacerplan'] ?? ''),
                    'route_id' => (string) ($item['idrota_salva'] ?? ''),
                ...
                <div class="calendar-quick-create" data-calendar-quick-create hidden>
                    ...
                    <section class="calendar-quick-create-popover" role="dialog" aria-modal="true" aria-labelledby="calendar-quick-create-title" data-quick-popover>
                        ...
                        <form data-calendar-quick-form>
                            ...
                            <details class="quick-create-details"><summary><?php echo stridebr_e(stridebr_t('schedule.more_details')); ?></summary><div class="quick-create-details-grid"><label><?php echo stridebr_e(stridebr_t('schedule.code')); ?><input type="text" name="codigo" maxlength="24" placeholder="<?php echo stridebr_e(stridebr_t('schedule.library_code_placeholder')); ?>" data-quick-code></label><label><?php echo stridebr_e(stridebr_t('schedule.focus')); ?><input type="text" name="foco" maxlength="80" placeholder="<?php echo stridebr_e(stridebr_t('schedule.library_focus_placeholder')); ?>" data-quick-focus></label><div class="quick-create-sport-field"><span class="form-field-label"><?php echo stridebr_e(stridebr_t('schedule.sport_activity')); ?></span><?php echo sportPickerRenderSelect($modalidadesTreino, ['name' => 'idmodalidade', 'empty_label' => 'Detectar automaticamente', 'native_attributes' => ['data-quick-modality' => true]]); ?></div><label data-quick-route-field>Rota<select name="route_id" data-quick-route><option value="">Sem rota</option><?php foreach ($savedRoutes as $route): ?><option value="<?php echo stridebr_e((string) $route['idrota_salva']); ?>" data-sport="<?php echo stridebr_e((string) ($route['idmodalidade'] ?? '')); ?>"><?php echo stridebr_e((string) $route['nome']); ?></option><?php endforeach; ?></select></label><label class="quick-create-description"><?php echo stridebr_e(stridebr_t('common.description')); ?><textarea name="descricao" rows="2" maxlength="500" data-quick-description></textarea></label></div></details>
