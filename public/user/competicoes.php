...
function competitionWebTimeToSeconds(string $value): ?int
{
    $value=trim($value);
...
function competitionWebSecondsToTime(mixed $value): string
{
    if(!is_numeric($value)||$value===null)return '';
    $seconds=max(0,(int)$value);$h=intdiv($seconds,3600);$m=intdiv($seconds%3600,60);$s=$seconds%60;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $payload = [
            ...
            'participacao_status' => $_POST['participacao_status'] ?? 'interessado',
            'resultado_tempo_s' => competitionWebTimeToSeconds((string)($_POST['resultado_tempo'] ?? '')),
            'resultado_distancia_m' => $_POST['resultado_distancia_m'] ?? '',
            'resultado_posicao_geral' => $_POST['resultado_posicao_geral'] ?? '',
            'resultado_posicao_categoria' => $_POST['resultado_posicao_categoria'] ?? '',
            'resultado_categoria' => $_POST['resultado_categoria'] ?? '',
            'resultado_medalha' => $_POST['resultado_medalha'] ?? '',
            'resultado_observacoes' => $_POST['resultado_observacoes'] ?? '',
            'url_oficial' => $_POST['url_oficial'] ?? '',
        ];
        if ($action === 'update') {
            $id = trim((string) ($_POST['idcompeticao'] ?? ''));
...
$history = array_values(array_filter($all, static fn(array $row): bool => !in_array($row, $upcoming, true)));
$institutionalCompetitions = stridebr_teams_enabled() ? stridebr_teams_surface_competitions($pdo, $idUsuario) : [];
...
<main class="main-content"><div class="page-shell competitions-page">
    ...
    <section class="content-card competition-form-card" aria-labelledby="competition-form-title">
        ...
        <form method="post" class="competition-form">
            ...
            <div class="competition-form-grid"><label><?php echo stridebr_e(stridebr_t('competitions.start_date')); ?><input type="date" name="data_inicio" required value="<?php echo stridebr_e((string)($form['data_inicio']??$today)); ?>"></label><label><?php echo stridebr_e(stridebr_t('competitions.end_date')); ?> <small><?php echo stridebr_e(stridebr_t('common.optional')); ?></small><input type="date" name="data_fim" value="<?php echo stridebr_e((string)($form['data_fim']??'')); ?>"></label><label><?php echo stridebr_e(stridebr_t('common.status')); ?><select name="status"><?php foreach(['planejada','realizada','cancelada'] as $status): ?><option value="<?php echo $status; ?>"<?php echo (string)($form['status']??'')===$status?' selected':''; ?>><?php echo stridebr_e(competitionStatusLabel($status)); ?></option><?php endforeach; ?></select></label><label>Participação<select name="participacao_status"><?php foreach(['interessado'=>'Interessado','inscrito'=>'Inscrito','participou'=>'Participou','cancelou'=>'Cancelou'] as $value=>$label): ?><option value="<?php echo $value; ?>"<?php echo (string)($form['participacao_status']??'interessado')===$value?' selected':''; ?>><?php echo $label; ?></option><?php endforeach; ?></select></label><label><?php echo stridebr_e(stridebr_t('competitions.main_sport')); ?> <small><?php echo stridebr_e(stridebr_t('common.optional')); ?></small><select name="idmodalidade_principal"><option value=""><?php echo stridebr_e(stridebr_t('common.not_informed')); ?></option><?php foreach($modalities as $modality): ?><option value="<?php echo stridebr_e((string)$modality['idmodalidade']); ?>"<?php echo (string)($form['idmodalidade_principal']??'')===(string)$modality['idmodalidade']?' selected':''; ?>><?php echo stridebr_e(stridebr_sport_name((string)($modality['slug']??''),(string)($modality['nome']??''))); ?></option><?php endforeach; ?></select></label></div>
            <details class="competition-form-more"<?php echo array_filter([(string)($form['local_nome']??''),(string)($form['cidade']??''),(string)($form['organizador']??''),(string)($form['observacoes']??''),(string)($form['oficialidade']??'')])? ' open':''; ?>><summary><?php echo stridebr_e(stridebr_t('competitions.optional_details')); ?></summary><div class="competition-form-grid"><label><?php echo stridebr_e(stridebr_t('competitions.location')); ?><input name="local_nome" maxlength="160" value="<?php echo stridebr_e((string)($form['local_nome']??'')); ?>"></label><label><?php echo stridebr_e(stridebr_t('competitions.city')); ?><input name="cidade" maxlength="100" value="<?php echo stridebr_e((string)($form['cidade']??'')); ?>"></label><label><?php echo stridebr_e(stridebr_t('competitions.state')); ?><input name="estado" maxlength="80" value="<?php echo stridebr_e((string)($form['estado']??'')); ?>"></label><label><?php echo stridebr_e(stridebr_t('competitions.country')); ?><input name="pais" maxlength="80" value="<?php echo stridebr_e((string)($form['pais']??'')); ?>"></label><label><?php echo stridebr_e(stridebr_t('competitions.organizer')); ?><input name="organizador" maxlength="160" value="<?php echo stridebr_e((string)($form['organizador']??'')); ?>"></label><label>Site oficial<input type="url" name="url_oficial" maxlength="500" value="<?php echo stridebr_e((string)($form['url_oficial']??'')); ?>" placeholder="https://"></label><label><?php echo stridebr_e(stridebr_t('competitions.level')); ?><input name="nivel" maxlength="80" value="<?php echo stridebr_e((string)($form['nivel']??'')); ?>"></label></div><label class="competition-official-check"><input type="checkbox" name="reported_official" value="1"<?php echo (string)($form['oficialidade']??'')==='informado_oficial'?' checked':''; ?>><span><strong><?php echo stridebr_e(stridebr_t('competitions.report_as_official')); ?></strong><small><?php echo stridebr_e(stridebr_t('competitions.report_as_official_help')); ?></small></span></label><fieldset class="competition-result-fields"><legend>Resultado</legend><div class="competition-form-grid"><label>Tempo<input name="resultado_tempo" value="<?php echo stridebr_e(competitionWebSecondsToTime($form['resultado_tempo_s']??null)); ?>" placeholder="52:10"></label><label>Distância <small>m</small><input type="number" min="1" step="0.01" name="resultado_distancia_m" value="<?php echo stridebr_e((string)($form['resultado_distancia_m']??'')); ?>"></label><label>Posição geral<input type="number" min="1" name="resultado_posicao_geral" value="<?php echo stridebr_e((string)($form['resultado_posicao_geral']??'')); ?>"></label><label>Posição categoria<input type="number" min="1" name="resultado_posicao_categoria" value="<?php echo stridebr_e((string)($form['resultado_posicao_categoria']??'')); ?>"></label><label>Categoria<input name="resultado_categoria" maxlength="120" value="<?php echo stridebr_e((string)($form['resultado_categoria']??'')); ?>"></label><label>Medalha<input name="resultado_medalha" maxlength="120" value="<?php echo stridebr_e((string)($form['resultado_medalha']??'')); ?>"></label></div><label>Observações do resultado<textarea name="resultado_observacoes" rows="2" maxlength="2000"><?php echo stridebr_e((string)($form['resultado_observacoes']??'')); ?></textarea></label></fieldset><label><?php echo stridebr_e(stridebr_t('competitions.notes')); ?><textarea name="observacoes" rows="3" maxlength="4000"><?php echo stridebr_e((string)($form['observacoes']??'')); ?></textarea></label></details>
    ...
    <article class="competition-detail">
        ...
        <div class="competition-facts"><div><span>Participação</span><strong><?php echo stridebr_e(ucfirst((string)($detailCompetition['participacao_status']??'interessado'))); ?></strong></div><div><span><?php echo stridebr_e(stridebr_t('competitions.activities')); ?></span><strong><?php echo (int)($detailCompetition['atividades_count']??0); ?></strong></div><div><span><?php echo stridebr_e(stridebr_t('competitions.marks_tests')); ?></span><strong><?php echo count($benchmarks); ?></strong></div><?php if(($label=competitionOfficialityLabel((string)$detailCompetition['oficialidade']))!==''): ?><div><span><?php echo stridebr_e(stridebr_t('competitions.officiality')); ?></span><strong><?php echo stridebr_e($label); ?></strong></div><?php endif; ?></div>
        <?php if (!empty($detailCompetition['resultado_tempo_s']) || !empty($detailCompetition['resultado_distancia_m']) || !empty($detailCompetition['resultado_posicao_geral']) || !empty($detailCompetition['resultado_medalha'])): ?><section class="competition-result-summary"><h3>Resultado</h3><div class="competition-facts"><?php if($detailCompetition['resultado_tempo_s']): ?><div><span>Tempo</span><strong><?php echo stridebr_e(competitionWebSecondsToTime($detailCompetition['resultado_tempo_s'])); ?></strong></div><?php endif; ?><?php if($detailCompetition['resultado_distancia_m']): ?><div><span>Distância</span><strong><?php echo stridebr_e(stridebr_format_number((float)$detailCompetition['resultado_distancia_m']/1000,2)); ?> km</strong></div><?php endif; ?><?php if($detailCompetition['resultado_posicao_geral']): ?><div><span>Geral</span><strong>#<?php echo (int)$detailCompetition['resultado_posicao_geral']; ?></strong></div><?php endif; ?><?php if($detailCompetition['resultado_posicao_categoria']): ?><div><span>Categoria</span><strong>#<?php echo (int)$detailCompetition['resultado_posicao_categoria']; ?><?php echo $detailCompetition['resultado_categoria']?' · '.stridebr_e((string)$detailCompetition['resultado_categoria']):''; ?></strong></div><?php endif; ?><?php if($detailCompetition['resultado_medalha']): ?><div><span>Medalha</span><strong><?php echo stridebr_e((string)$detailCompetition['resultado_medalha']); ?></strong></div><?php endif; ?></div><?php if($detailCompetition['resultado_observacoes']): ?><p><?php echo nl2br(stridebr_e((string)$detailCompetition['resultado_observacoes'])); ?></p><?php endif; ?></section><?php endif; ?>
        <?php if (!empty($detailCompetition['url_oficial'])): ?><p class="competition-related-event"><a href="<?php echo stridebr_e((string)$detailCompetition['url_oficial']); ?>" rel="noopener noreferrer" target="_blank">Site oficial</a></p><?php endif; ?>
        ...
        <?php if ((string)($detailCompetition['status']??'') === 'planejada'): ?><section class="competition-detail-section"><header><h3>Preparação</h3></header><div class="competition-prep-actions"><a class="secondary-button" href="/user/cronogramatreinos.php?competition=<?php echo rawurlencode((string)$detailCompetition['idcompeticao']); ?>">Treinos</a><a class="secondary-button" href="/user/pacer.php?competition=<?php echo rawurlencode((string)$detailCompetition['idcompeticao']); ?>">Pacer</a><a class="secondary-button" href="/user/progresso.php">Metas</a></div></section><?php endif; ?>
    ...
    <?php if ($institutionalCompetitions !== []): ?>
    <section class="competitions-list-section institutional-card"><header><div><span class="institutional-badge"><?php echo stridebr_e(stridebr_t('teams.read_only')); ?></span><h2><?php echo stridebr_e(stridebr_t('teams.institutional_competitions')); ?></h2></div></header><div class="competitions-list"><?php foreach ($institutionalCompetitions as $competition): ?><a href="/user/competicao-institucional.php?id=<?php echo rawurlencode((string) $competition['competition_ref']); ?>"><div><strong><?php echo stridebr_e((string) $competition['display_name']); ?></strong><small><?php echo stridebr_e((string) $competition['start_date']); ?> — <?php echo stridebr_e((string) $competition['end_date']); ?></small></div><span><?php echo !empty($competition['delegation_member']) ? stridebr_e(stridebr_t('teams.delegation')) : ''; ?></span></a><?php endforeach; ?></div></section>
    <?php endif; ?>
    <?php if ($all === [] && !$isFormOpen): ?>
        <section class="competition-empty-state" aria-labelledby="competition-empty-title"><h2 id="competition-empty-title"><?php echo stridebr_e(stridebr_t('competitions.empty')); ?></h2><p><?php echo stridebr_e(stridebr_t('competitions.empty_help')); ?></p><a class="primary-button" href="/user/competicoes.php?new=1"><?php echo stridebr_e(stridebr_t('competitions.register')); ?></a></section>
    <?php else: ?>
        <?php if ($upcoming !== []): ?><section class="competitions-list-section"><header><h2><?php echo stridebr_e(stridebr_t('competitions.upcoming')); ?></h2></header><div class="competitions-list"><?php foreach($upcoming as $competition): ?><a href="/user/competicoes.php?id=<?php echo rawurlencode((string)$competition['idcompeticao']); ?>"><div><strong><?php echo stridebr_e((string)$competition['nome']); ?></strong><small><?php echo !empty($competition['modalidade_nome']) ? stridebr_e((string)$competition['modalidade_nome']) . ' · ' : ''; ?><?php echo stridebr_e($formatDateRange($competition)); ?><?php $loc=implode(', ',array_filter([(string)($competition['cidade']??''),(string)($competition['estado']??'')])); if($loc!==''): ?> · <?php echo stridebr_e($loc); ?><?php endif; ?></small></div><span><?php echo stridebr_e(competitionStatusLabel((string)$competition['status'])); ?></span></a><?php endforeach; ?></div></section><?php endif; ?>
        <?php if ($history !== []): ?><section class="competitions-list-section"><header><h2><?php echo stridebr_e(stridebr_t('competitions.history')); ?></h2></header><div class="competitions-list"><?php foreach($history as $competition): ?><a href="/user/competicoes.php?id=<?php echo rawurlencode((string)$competition['idcompeticao']); ?>"><div><strong><?php echo stridebr_e((string)$competition['nome']); ?></strong><small><?php echo !empty($competition['modalidade_nome']) ? stridebr_e((string)$competition['modalidade_nome']) . ' · ' : ''; ?><?php echo stridebr_e($formatDateRange($competition)); ?> · <?php echo (int)($competition['atividades_count']??0); ?> atividades<?php if ((int)($competition['benchmarks_count']??0) > 0): ?> · <?php echo (int)$competition['benchmarks_count']; ?> marcas/testes<?php endif; ?></small></div><span><?php echo stridebr_e(competitionStatusLabel((string)$competition['status'])); ?></span></a><?php endforeach; ?></div></section><?php endif; ?>
    <?php endif; ?>
</div></main>
