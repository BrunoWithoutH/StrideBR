...
<!DOCTYPE html><html lang="<?php echo function_exists('stridebr_html_lang') ? stridebr_e(stridebr_html_lang()) : 'pt-BR'; ?>"><head>
    <?php if (function_exists('stridebr_ui_boot_script')) echo stridebr_ui_boot_script(); ?><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><link rel="icon" type="image/png" href="<?php echo stridebr_e(stridebr_asset('/assets/img/favicon/favicon.png')); ?>"><link rel="stylesheet" href="<?php echo stridebr_e(stridebr_asset('/assets/css/style.css')); ?>"><link rel="stylesheet" href="<?php echo stridebr_e(stridebr_asset('/assets/css/product-insights.css')); ?>"><title><?php echo stridebr_e(stridebr_t('compare.page_title')); ?> | StrideBR</title><link rel="stylesheet" href="<?php echo stridebr_e(stridebr_asset('/assets/css/ui-refresh.css')); ?>">
</head><body><div class="container-fluid">
<?php require dirname(__DIR__, 2) . '/src/layout/header.php'; ?>
<main class="main-content product-page">
<header class="product-page-header"><div><h1><?php echo stridebr_e(stridebr_t('common.compare_activities')); ?></h1></div><div class="product-toolbar"><a class="product-button-secondary" href="/user/atividades.php"><?php echo stridebr_e(stridebr_t('activity.history')); ?></a><a class="product-button-secondary" href="/user/progresso.php"><?php echo stridebr_e(stridebr_t('nav.progress')); ?></a></div></header>
<?php if (count($activities) < 2): ?>
<section class="empty-action-state"><h2><?php echo stridebr_e(stridebr_t('compare.need_more')); ?></h2><a class="product-button" href="/user/atividades.php?new=1"><?php echo stridebr_e(stridebr_t('progress.log_activity')); ?></a></section>
<?php else: ?>
<section class="insight-section"><form class="compare-form" method="get"><label><?php echo stridebr_e(stridebr_t('compare.activity_a')); ?><select name="a" onchange="this.form.submit()"><?php foreach ($activities as $row): ?><option value="<?php echo stridebr_e((string) $row['idregistro']); ?>"<?php echo (string) $row['idregistro'] === $idA ? ' selected' : ''; ?>><?php echo stridebr_e($formatActivity($row)); ?></option><?php endforeach; ?></select></label><span class="compare-vs">VS.</span><label><?php echo stridebr_e(stridebr_t('compare.activity_b')); ?><select name="b"><?php foreach ($activities as $row): if ((string) $row['idregistro'] === $idA) continue; ?><option value="<?php echo stridebr_e((string) $row['idregistro']); ?>"<?php echo (string) $row['idregistro'] === $idB ? ' selected' : ''; ?>><?php echo stridebr_e($formatActivity($row)); ?></option><?php endforeach; ?></select></label><button class="product-button" type="submit"><?php echo stridebr_e(stridebr_t('activity.compare')); ?></button></form>
<?php if ($suggestions): ?><div class="compare-suggestions" aria-label="<?php echo stridebr_e(stridebr_t('compare.suggestions')); ?>"><?php foreach ($suggestions as $suggestion): $row=$suggestion['activity']; ?><a class="compare-suggestion" href="?a=<?php echo rawurlencode($idA); ?>&amp;b=<?php echo rawurlencode((string) $row['idregistro']); ?>"><strong><?php echo stridebr_e((string) $suggestion['label']); ?></strong><span><?php echo stridebr_e($formatActivity($row)); ?></span></a><?php endforeach; ?></div><?php endif; ?>
</section>
...
<div class="compare-head"><div class="compare-axis"><span class="insight-muted"><?php echo stridebr_e(stridebr_t('compare.comparison')); ?></span></div><article><small><?php echo stridebr_e(stridebr_t('compare.activity_a')); ?></small><strong><?php echo stridebr_e((string) $a['titulo']); ?></strong><span><?php echo stridebr_e(stridebr_sport_name((string) ($a['modalidade_slug'] ?? ''), (string) $a['modalidade'])); ?> · <?php echo stridebr_e((string) $a['data']); ?></span></article><article><small><?php echo stridebr_e(stridebr_t('compare.activity_b')); ?></small><strong><?php echo stridebr_e((string) $b['titulo']); ?></strong><span><?php echo stridebr_e(stridebr_sport_name((string) ($b['modalidade_slug'] ?? ''), (string) $b['modalidade'])); ?> · <?php echo stridebr_e((string) $b['data']); ?></span></article></div>
<section class="insight-section"><header><div><h2><?php echo stridebr_e(stridebr_t('settings.metric_units')); ?></h2></div></header><table class="compare-table"><tbody><tr><td><?php echo stridebr_e(stridebr_t('activity.perceived_effort')); ?></td><td><?php echo $a['esforco'] !== null ? (int)$a['esforco'].'/10' : '—'; ?></td><td><?php echo $b['esforco'] !== null ? (int)$b['esforco'].'/10' : '—'; ?></td></tr><?php foreach ($comparison['metricas'] as $metric): ?><tr><td><?php echo stridebr_e((string) $metric['rotulo']); ?></td><td><?php echo stridebr_e((string) $metric['a']); ?></td><td><?php echo stridebr_e((string) $metric['b']); ?></td></tr><?php endforeach; ?><?php if ($a['rota'] || $b['rota']): ?><tr><td><?php echo stridebr_e(stridebr_t('compare.route_gain')); ?></td><td><?php echo isset($a['rota']['ganho_m']) && $a['rota']['ganho_m'] !== null ? $formatNumber((float)$a['rota']['ganho_m']).' m' : '—'; ?></td><td><?php echo isset($b['rota']['ganho_m']) && $b['rota']['ganho_m'] !== null ? $formatNumber((float)$b['rota']['ganho_m']).' m' : '—'; ?></td></tr><?php endif; ?></tbody></table></section>
