...
<main class="main-content"><div class="page-shell friends-shell">
    ...
    <?php if ($search !== ''): ?><section class="friend-section"><h2><?php echo stridebr_e(stridebr_t('friends.results')); ?></h2><div class="people-grid">
        ...
        <?php foreach ($searchResults as $user): ?><?php $profileUrl = '/u/' . rawurlencode((string) $user['username']); ?><article class="person-card"><a class="person-identity-link" href="<?php echo stridebr_e($profileUrl); ?>"><img src="<?php echo stridebr_e(friendAvatar($user)); ?>" alt="" width="48" height="48" loading="lazy" decoding="async"><span><strong><?php echo stridebr_e(stridebr_person_name_for_display((string)$user['nome_exibicao'], (string)($user['username'] ?? ''), stridebr_t('common.user'), 60)); ?></strong><?php if ($user['username']): ?><small>@<?php echo stridebr_e($user['username']); ?></small><?php endif; ?></span></a>
