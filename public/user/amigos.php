...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if (in_array($action, ['send', 'accept', 'reject', 'cancel_request', 'remove'], true) && ($target === '' || $target === $idUsuario)) throw new InvalidArgumentException(stridebr_t('friends.invalid_user'));
        if ($action === 'send') {
            ...
            $stmt = $pdo->prepare("DELETE FROM amizades WHERE idusuario_solicitante = :me AND idusuario_destino = :target AND status = 'pendente'");
            $stmt->execute([':me' => $idUsuario, ':target' => $target]);
            if ($stmt->rowCount() !== 1) throw new RuntimeException(stridebr_t('friends.request_not_found'));
            stridebr_flash('info', stridebr_t('friends.request_cancelled'));
...
$outgoingStmt = $pdo->prepare("SELECT u.idusuario, u.username, COALESCE(NULLIF(u.nome_exibicao, ''), u.nomeusuario) AS nome_exibicao, u.fotousuario FROM amizades a JOIN usuarios u ON u.idusuario = a.idusuario_destino WHERE a.idusuario_solicitante = :me AND a.status = 'pendente' ORDER BY a.data_criacao DESC");
$outgoingStmt->execute([':me' => $idUsuario]);
$outgoing = $outgoingStmt->fetchAll();

$friends = [];
$friendsLoadError = false;
try {
...
$friendsView = (string) ($_GET['view'] ?? 'people');
if (!in_array($friendsView, ['people', 'sharing'], true)) $friendsView = 'people';
...
<main class="main-content"><div class="page-shell friends-shell">
    <div class="page-heading"><h1><?php echo stridebr_e(stridebr_t('friends.page_title')); ?></h1><a class="secondary-button" href="#friend-q"><?php echo stridebr_e(stridebr_t('friends.search_people')); ?></a></div>
    ...
    <nav class="ux-context-nav friends-context-nav" aria-label="<?php echo stridebr_e(stridebr_t('friends.navigation')); ?>">
        <a class="<?php echo $friendsView === 'people' ? 'is-active' : ''; ?>" href="/user/amigos.php?view=people"><?php echo stridebr_e(stridebr_t('friends.people')); ?></a>
        <a class="<?php echo $friendsView === 'sharing' ? 'is-active' : ''; ?>" href="/user/amigos.php?view=sharing"><?php echo stridebr_e(stridebr_t('friends.sharing')); ?></a>
    </nav>

    <?php if ($friendsView === 'people'): ?>
    <div class="friends-people-stack">
        <form method="GET" class="friend-search compact-search"><input type="hidden" name="view" value="people"><label for="friend-q"><?php echo stridebr_e(stridebr_t('friends.search_person')); ?></label><div><input id="friend-q" type="search" name="q" value="<?php echo stridebr_e($search); ?>" placeholder="<?php echo stridebr_e(stridebr_t('friends.search_placeholder')); ?>"><button type="submit"><?php echo stridebr_e(stridebr_t('common.search')); ?></button></div></form>
        <?php if ($incoming !== []): ?><section class="friend-section"><div class="section-title-row"><h2><?php echo stridebr_e(stridebr_t('friends.incoming_requests')); ?></h2><span><?php echo count($incoming); ?></span></div><div class="people-grid"><?php foreach ($incoming as $user): $profileUrl = '/u/' . rawurlencode((string) $user['username']); ?><article class="person-card"><a class="person-identity-link" href="<?php echo stridebr_e($profileUrl); ?>"><img src="<?php echo stridebr_e(friendAvatar($user)); ?>" alt="" width="48" height="48" loading="lazy" decoding="async"><span><strong><?php echo stridebr_e(stridebr_person_name_for_display((string)$user['nome_exibicao'], (string)($user['username'] ?? ''), stridebr_t('common.user'), 60)); ?></strong><?php if ($user['username']): ?><small>@<?php echo stridebr_e($user['username']); ?></small><?php endif; ?></span></a><div class="person-actions"><form method="POST"><?php echo stridebr_csrf_field(); ?><input type="hidden" name="action" value="accept"><input type="hidden" name="idusuario" value="<?php echo stridebr_e($user['idusuario']); ?>"><button type="submit"><?php echo stridebr_e(stridebr_t('friends.accept')); ?></button></form><form method="POST"><?php echo stridebr_csrf_field(); ?><input type="hidden" name="action" value="reject"><input type="hidden" name="idusuario" value="<?php echo stridebr_e($user['idusuario']); ?>"><button type="submit" class="quiet"><?php echo stridebr_e(stridebr_t('friends.decline')); ?></button></form></div></article><?php endforeach; ?></div></section><?php endif; ?>
        ...
        <section class="friend-section"><div class="section-title-row"><h2><?php echo stridebr_e(stridebr_t('friends.your_friends')); ?></h2><span><?php echo count($friends); ?></span></div><div class="people-grid">
            <?php if ($friendsLoadError): ?><div class="alert alert-danger" role="alert"><?php echo stridebr_e(stridebr_t('friends.load_error')); ?></div><?php elseif ($friends === []): ?><div class="content-card"><strong><?php echo stridebr_e(stridebr_t('friends.empty')); ?></strong><a href="#friend-q"><?php echo stridebr_e(stridebr_t('friends.search_people')); ?></a></div><?php endif; ?>
            <?php foreach ($friends as $user): $profileUrl = '/u/' . rawurlencode((string) $user['username']); ?><article class="person-card"><a class="person-identity-link" href="<?php echo stridebr_e($profileUrl); ?>"><img src="<?php echo stridebr_e(friendAvatar($user)); ?>" alt="" width="48" height="48" loading="lazy" decoding="async"><span><strong><?php echo stridebr_e(stridebr_person_name_for_display((string)$user['nome_exibicao'], (string)($user['username'] ?? ''), stridebr_t('common.user'), 60)); ?></strong><?php if ($user['username']): ?><small>@<?php echo stridebr_e($user['username']); ?></small><?php endif; ?></span></a><div class="friend-primary-actions"><a class="person-profile-action" href="<?php echo stridebr_e($profileUrl); ?>"><?php echo stridebr_e(stridebr_t('friends.open_profile')); ?></a><details class="person-action-menu"><summary aria-label="<?php echo stridebr_e(stridebr_t('friends.person_actions')); ?>"><svg class="ui-icon" viewBox="0 0 24 24" aria-hidden="true"><circle cx="5" cy="12" r="1"></circle><circle cx="12" cy="12" r="1"></circle><circle cx="19" cy="12" r="1"></circle></svg></summary><div><a href="<?php echo stridebr_e($profileUrl); ?>"><?php echo stridebr_e(stridebr_t('friends.open_profile')); ?></a><a href="/user/amigos.php?view=sharing#cronogramas-sincronizados"><?php echo stridebr_e(stridebr_t('friends.sharing')); ?></a><form method="POST" data-confirm="<?php echo stridebr_e(stridebr_t('friends.remove_confirm')); ?>"><?php echo stridebr_csrf_field(); ?><input type="hidden" name="action" value="remove"><input type="hidden" name="idusuario" value="<?php echo stridebr_e($user['idusuario']); ?>"><button type="submit" class="is-danger"><?php echo stridebr_e(stridebr_t('friends.remove')); ?></button></form></div></details></div></article><?php endforeach; ?>
        </div></section>

        <section class="friend-section">
            <?php if ($search !== ''): ?><div class="people-grid"><?php if ($searchResults === []): ?><div class="content-card"><strong><?php echo stridebr_e(stridebr_t('friends.no_results')); ?></strong></div><?php endif; ?><?php foreach ($searchResults as $user): $profileUrl = '/u/' . rawurlencode((string) $user['username']); ?><article class="person-card"><a class="person-identity-link" href="<?php echo stridebr_e($profileUrl); ?>"><img src="<?php echo stridebr_e(friendAvatar($user)); ?>" alt="" width="48" height="48"><span><strong><?php echo stridebr_e(stridebr_person_name_for_display((string)$user['nome_exibicao'], (string)($user['username'] ?? ''), stridebr_t('common.user'), 60)); ?></strong><?php if ($user['username']): ?><small>@<?php echo stridebr_e($user['username']); ?></small><?php endif; ?></span></a><?php if (!$user['amizade_status']): ?><form method="POST"><?php echo stridebr_csrf_field(); ?><input type="hidden" name="action" value="send"><input type="hidden" name="idusuario" value="<?php echo stridebr_e($user['idusuario']); ?>"><button type="submit"><?php echo stridebr_e(stridebr_t('friends.add')); ?></button></form><?php else: ?><span class="relationship-badge"><?php echo stridebr_e($user['amizade_status'] === 'aceita' ? stridebr_t('friends.accepted') : stridebr_t('friends.pending')); ?></span><?php endif; ?></article><?php endforeach; ?></div><?php endif; ?>
        </section>

        <?php if ($outgoing !== []): ?><section class="friend-section"><div class="section-title-row"><h2><?php echo stridebr_e(stridebr_t('friends.outgoing_requests')); ?></h2><span><?php echo count($outgoing); ?></span></div><div class="people-grid"><?php foreach ($outgoing as $user): $profileUrl = '/u/' . rawurlencode((string) $user['username']); ?><article class="person-card"><a class="person-identity-link" href="<?php echo stridebr_e($profileUrl); ?>"><img src="<?php echo stridebr_e(friendAvatar($user)); ?>" alt="" width="48" height="48"><span><strong><?php echo stridebr_e(stridebr_person_name_for_display((string)$user['nome_exibicao'], (string)($user['username'] ?? ''), stridebr_t('common.user'), 60)); ?></strong><?php if ($user['username']): ?><small>@<?php echo stridebr_e($user['username']); ?></small><?php endif; ?></span></a><div class="friend-outgoing-status"><span><?php echo stridebr_e(stridebr_t('friends.awaiting_response')); ?></span><form method="POST" data-confirm="<?php echo stridebr_e(stridebr_t('friends.cancel_request_confirm')); ?>"><?php echo stridebr_csrf_field(); ?><input type="hidden" name="action" value="cancel_request"><input type="hidden" name="idusuario" value="<?php echo stridebr_e($user['idusuario']); ?>"><button type="submit" class="quiet"><?php echo stridebr_e(stridebr_t('friends.cancel_request')); ?></button></form></div></article><?php endforeach; ?></div></section><?php endif; ?>
    </div>
    <?php else: ?>
    <div class="friends-sharing-panel">
