...
<main class="main-content"><div class="page-shell friends-shell">
    ...
    <div class="friends-people-stack">
        ...
        <section class="friend-section"><div class="section-title-row"><h2><?php echo stridebr_e(stridebr_t('friends.your_friends')); ?></h2><span><?php echo count($friends); ?></span></div><div class="people-grid">
            <?php if ($friends === []): ?><div class="content-card"><strong><?php echo stridebr_e(stridebr_t('friends.empty')); ?></strong><p><?php echo stridebr_e(stridebr_t('friends.empty_people_help')); ?></p><a href="#friend-q"><?php echo stridebr_e(stridebr_t('friends.search_people')); ?></a></div><?php endif; ?>
            <?php foreach ($friends as $user): $profileUrl = '/u/' . rawurlencode((string) $user['username']); ?><article class="person-card"><a class="person-identity-link" href="<?php echo stridebr_e($profileUrl); ?>"><img src="<?php echo stridebr_e(friendAvatar($user)); ?>" alt="" width="48" height="48" loading="lazy" decoding="async"><span><strong><?php echo stridebr_e(stridebr_person_name_for_display((string)$user['nome_exibicao'], (string)($user['username'] ?? ''), stridebr_t('common.user'), 60)); ?></strong><?php if ($user['username']): ?><small>@<?php echo stridebr_e($user['username']); ?></small><?php endif; ?></span></a><details class="person-action-menu"><summary aria-label="<?php echo stridebr_e(stridebr_t('friends.person_actions')); ?>"><svg class="ui-icon" viewBox="0 0 24 24" aria-hidden="true"><circle cx="5" cy="12" r="1"></circle><circle cx="12" cy="12" r="1"></circle><circle cx="19" cy="12" r="1"></circle></svg></summary><div><a href="<?php echo stridebr_e($profileUrl); ?>"><?php echo stridebr_e(stridebr_t('friends.open_profile')); ?></a><a href="/user/amigos.php?view=sharing#cronogramas-sincronizados"><?php echo stridebr_e(stridebr_t('friends.sharing')); ?></a><form method="POST" data-confirm="<?php echo stridebr_e(stridebr_t('friends.remove_confirm')); ?>"><?php echo stridebr_csrf_field(); ?><input type="hidden" name="action" value="remove"><input type="hidden" name="idusuario" value="<?php echo stridebr_e($user['idusuario']); ?>"><button type="submit" class="is-danger"><?php echo stridebr_e(stridebr_t('friends.remove')); ?></button></form></div></details></article><?php endforeach; ?>
        </div></section>

        <section class="friend-section"><div class="section-title-row"><div><h2><?php echo stridebr_e(stridebr_t('friends.search_people')); ?></h2><p><?php echo stridebr_e(stridebr_t('friends.search_people_help')); ?></p></div></div><form method="GET" class="friend-search compact-search content-card"><input type="hidden" name="view" value="people"><label for="friend-q"><?php echo stridebr_e(stridebr_t('friends.search_person')); ?></label><div><input id="friend-q" type="search" name="q" value="<?php echo stridebr_e($search); ?>" placeholder="<?php echo stridebr_e(stridebr_t('friends.search_placeholder')); ?>"><button type="submit"><?php echo stridebr_e(stridebr_t('common.search')); ?></button></div></form>
            <?php if ($search !== ''): ?><div class="people-grid"><?php if ($searchResults === []): ?><div class="content-card"><strong><?php echo stridebr_e(stridebr_t('friends.no_results')); ?></strong><p><?php echo stridebr_e(stridebr_t('friends.no_results_help')); ?></p></div><?php endif; ?><?php foreach ($searchResults as $user): $profileUrl = '/u/' . rawurlencode((string) $user['username']); ?><article class="person-card"><a class="person-identity-link" href="<?php echo stridebr_e($profileUrl); ?>"><img src="<?php echo stridebr_e(friendAvatar($user)); ?>" alt="" width="48" height="48"><span><strong><?php echo stridebr_e(stridebr_person_name_for_display((string)$user['nome_exibicao'], (string)($user['username'] ?? ''), stridebr_t('common.user'), 60)); ?></strong><?php if ($user['username']): ?><small>@<?php echo stridebr_e($user['username']); ?></small><?php endif; ?></span></a><?php if (!$user['amizade_status']): ?><form method="POST"><?php echo stridebr_csrf_field(); ?><input type="hidden" name="action" value="send"><input type="hidden" name="idusuario" value="<?php echo stridebr_e($user['idusuario']); ?>"><button type="submit"><?php echo stridebr_e(stridebr_t('friends.add')); ?></button></form><?php else: ?><span class="relationship-badge"><?php echo stridebr_e($user['amizade_status'] === 'aceita' ? stridebr_t('friends.accepted') : stridebr_t('friends.pending')); ?></span><?php endif; ?></article><?php endforeach; ?></div><?php endif; ?>
        </section>
