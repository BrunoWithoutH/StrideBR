...
foreach ($benchmarkRows as $row) {
    if ((string) ($row['tipo'] ?? '') === 'athletics') continue;
    $benchmarkClientRows[] = [
...
if ($available && benchmarkTableExists($pdo)) {
    foreach (benchmarkAthleticsEvidence($pdo, $idUsuario) as $row) {
        $eligibility = is_array($row['record_eligibility'] ?? null) ? $row['record_eligibility'] : athleticsRecordEligibility($row);
        $benchmarkClientRows[] = [
            'id' => (string) ($row['idbenchmark'] ?? $row['idregistro'] ?? ''), 'type' => 'athletics', 'sport' => (string) ($row['idmodalidade'] ?? ''), 'exercise' => '', 'distance' => null,
            'event' => (string) ($row['athletics_event_code'] ?? ''), 'environment' => athleticsNormalizeEnvironment($row['athletics_environment'] ?? 'unknown'), 'eligibility' => (string) ($eligibility['status'] ?? 'unknown'),
            'value' => is_numeric($row['valor_canonico'] ?? null) ? (float) $row['valor_canonico'] : null, 'date' => (string) ($row['data_resultado'] ?? ''),
        ];
    }
}
...
$benchmarkClientRegistry['athletics']['events'] = [];
foreach (athleticsCatalog() as $code => $event) {
    if (!isset($athleticsGoalModalities[$code])) continue;
    $benchmarkClientRegistry['athletics']['events'][$code] = [
        'label' => athleticsEventLabel($code), 'measurement' => (string) $event['measurement'], 'direction' => (string) $event['direction'], 'sport' => (string) $athleticsGoalModalities[$code]['idmodalidade'],
    ];
}
        ...
        <div class="page-shell goals-page">
            ...
            <?php else: ?>
                <?php if ($showForm):
                    ...
                    $formBenchmarkEvent = (string) ($form['benchmark_event_code'] ?? ($_POST['benchmark_event_code'] ?? ''));
                    $formBenchmarkEnvironment = athleticsNormalizeEnvironment($form['benchmark_environment'] ?? ($_POST['benchmark_environment'] ?? 'unknown'));
                    $formBenchmarkRequireEligible = array_key_exists('benchmark_require_eligible', $form) ? stridebr_db_bool($form['benchmark_require_eligible']) : isset($_POST['benchmark_require_eligible']);
                    ...
                    $formTarget = (string) $rawTarget;
                    if ($formType === 'benchmark' && is_numeric($rawTarget)) {
                        if (in_array($formBenchmarkType, ['css', 'distance_time'], true)) $formTarget = benchmarkFormatClock((float) $rawTarget);
                        elseif ($formBenchmarkType === 'athletics' && (athleticsEventConfig($formBenchmarkEvent)['measurement'] ?? '') === 'time') $formTarget = athleticsFormatTime((float) $rawTarget);
                    }
                    $formTargetUnit = '';
                    if ($formType === 'benchmark' && $formBenchmarkType !== '') {
                        if ($formBenchmarkType === 'athletics') $formTargetUnit = (athleticsEventConfig($formBenchmarkEvent)['measurement'] ?? '') === 'time' ? 's' : 'm';
                        else $formTargetUnit = (string) (benchmarkTypeConfig($formBenchmarkType)['unit'] ?? '');
                    } else $formTargetUnit = dashboardMetaUnidade($formMetric);
                            ...
                            <div class="goals-form-layout">
                                <section class="goals-form-block" aria-labelledby="goal-track-heading">
                                    ...
                                    <div data-goal-benchmark-fields<?php echo $formType === 'benchmark' ? '' : ' hidden'; ?>>
                                        ...
                                        <div class="goal-athletics-fields" data-goal-athletics-fields<?php echo $formBenchmarkType === 'athletics' ? '' : ' hidden'; ?>>
                                            <label><?php echo stridebr_e(stridebr_t('athletics.event')); ?><select name="benchmark_event_code" data-goal-athletics-event<?php echo $editMeta && $formType === 'benchmark' ? ' data-goal-locked="1" disabled' : ''; ?>><option value=""><?php echo stridebr_e(stridebr_t('goals.benchmark.choose_athletics_event')); ?></option><?php foreach (athleticsCatalog() as $code => $event): if (!isset($athleticsGoalModalities[$code])) continue; ?><option value="<?php echo stridebr_e($code); ?>" data-measurement="<?php echo stridebr_e((string)$event['measurement']); ?>" data-direction="<?php echo stridebr_e((string)$event['direction']); ?>" data-sport="<?php echo stridebr_e((string)$athleticsGoalModalities[$code]['idmodalidade']); ?>"<?php echo $formBenchmarkEvent === $code ? ' selected' : ''; ?>><?php echo stridebr_e(athleticsEventLabel($code)); ?></option><?php endforeach; ?></select><?php if ($editMeta && $formType === 'benchmark'): ?><input type="hidden" name="benchmark_event_code" value="<?php echo stridebr_e($formBenchmarkEvent); ?>"><?php endif; ?></label>
                                            <label><?php echo stridebr_e(stridebr_t('athletics.environment.label')); ?><select name="benchmark_environment" data-goal-athletics-environment<?php echo $editMeta && $formType === 'benchmark' ? ' data-goal-locked="1" disabled' : ''; ?>><?php foreach (['unknown','outdoor','indoor'] as $environment): ?><option value="<?php echo stridebr_e($environment); ?>"<?php echo $formBenchmarkEnvironment === $environment ? ' selected' : ''; ?>><?php echo stridebr_e(stridebr_t('athletics.environment.' . $environment)); ?></option><?php endforeach; ?></select><?php if ($editMeta && $formType === 'benchmark'): ?><input type="hidden" name="benchmark_environment" value="<?php echo stridebr_e($formBenchmarkEnvironment); ?>"><?php endif; ?></label>
                                            <label class="goal-inline-check"><input type="checkbox" name="benchmark_require_eligible" value="1" data-goal-athletics-eligible<?php echo $formBenchmarkRequireEligible ? ' checked' : ''; ?><?php echo $editMeta && $formType === 'benchmark' ? ' data-goal-locked="1" disabled' : ''; ?>><span><?php echo stridebr_e(stridebr_t('goals.benchmark.require_eligible')); ?></span></label>
                                            <?php if ($editMeta && $formType === 'benchmark' && $formBenchmarkRequireEligible): ?><input type="hidden" name="benchmark_require_eligible" value="1"><?php endif; ?>
                                        </div>
                                    </div>
                ...
                <section class="goals-section" data-goals-section="active">
                    ...
                    <div class="goals-grid">
                        ...
                        <article class="goal-card goal-card-benchmark<?php echo !empty($meta['atingida'])?' is-complete':''; ?><?php echo !empty($meta['expirada'])?' is-expired':''; ?>">
                            ...
                            <div class="goal-benchmark-values"><div><span><?php echo stridebr_e(stridebr_t('goals.benchmark.best_since_start')); ?></span><strong><?php echo $hasResult ? stridebr_e(benchmarkGoalFormatValue($type,$bestValue,is_numeric($meta['benchmark_distancia_m']??null)?(float)$meta['benchmark_distancia_m']:null,(string)($meta['benchmark_event_code']??''))) : '—'; ?></strong></div><div><span><?php echo stridebr_e(stridebr_t('goals.target')); ?></span><strong><?php echo stridebr_e(benchmarkGoalFormatValue($type,$target,is_numeric($meta['benchmark_distancia_m']??null)?(float)$meta['benchmark_distancia_m']:null,(string)($meta['benchmark_event_code']??''))); ?></strong></div></div>
                            <?php if (is_numeric($meta['valor_inicial']??null)): ?><div class="goal-benchmark-baseline"><span><?php echo stridebr_e(stridebr_t('goals.benchmark.at_creation')); ?></span><strong><?php echo stridebr_e(benchmarkGoalFormatValue($type,(float)$meta['valor_inicial'],is_numeric($meta['benchmark_distancia_m']??null)?(float)$meta['benchmark_distancia_m']:null,(string)($meta['benchmark_event_code']??''))); ?></strong></div><?php endif; ?>
                            <div class="goal-status"><span><?php echo stridebr_e(dashboardMetaPrazoLabel($meta)); ?></span><strong><?php if (!empty($meta['atingida'])) echo stridebr_e(stridebr_t('goals.benchmark.completed')); elseif (!empty($meta['expirada'])) echo stridebr_e(stridebr_t('goals.benchmark.expired')); elseif (!$hasResult) echo stridebr_e(match($type){'one_rm'=>stridebr_t('goals.benchmark.no_one_rm'),'ftp'=>stridebr_t('goals.benchmark.no_ftp'),'css'=>stridebr_t('goals.benchmark.no_css'),'distance_time'=>stridebr_t('goals.benchmark.no_test'),'athletics'=>stridebr_t('goals.benchmark.no_athletics'),default=>stridebr_t('goals.benchmark.no_result')}); else echo stridebr_e(stridebr_t('goals.benchmark.remaining') . ': ' . benchmarkGoalFormatDifference($type,$remaining,(string)($meta['benchmark_event_code']??''))); ?></strong><?php if (!$hasResult): ?><a href="<?php echo stridebr_e(benchmarkGoalResultHref($meta)); ?>"><?php echo stridebr_e(stridebr_t('goals.benchmark.register_result')); ?></a><?php elseif (!$percentAvailable && empty($meta['atingida'])): ?><small><?php echo stridebr_e(stridebr_t('goals.benchmark.progress_unavailable')); ?></small><?php endif; ?></div>
