...
function stridebr_pacer_web_seconds(string $value): ?float
{
    ...
    if (!preg_match('/^\d{1,3}:\d{1,2}(?::\d{1,2})?$/', $value)) return null;
    $parts = array_map('intval', explode(':', $value));
    if (count($parts) === 2 && $parts[1] < 60) return $parts[0] * 60 + $parts[1];
    if (count($parts) === 3 && $parts[1] < 60 && $parts[2] < 60) return $parts[0] * 3600 + $parts[1] * 60 + $parts[2];
    return null;
}
...
<main class="main-content pacer-page" data-pacer-page data-csrf="<?php echo stridebr_e($csrf); ?>">
    ...
    <div class="pacer-layout<?php echo !$hasAnyPlans ? ' is-first-use' : ''; ?>">
        ...
        <section class="pacer-editor">
            <form method="post" data-pacer-form>
                ...
                <div class="pacer-editor-head"><div><span><?php echo $editing ? 'Editar estratégia' : 'Nova estratégia'; ?></span><strong><?php echo stridebr_e((string) ($editing['name'] ?? (!$hasAnyPlans ? 'Defina distância, tempo e estratégia' : 'Plano de pace'))); ?></strong></div><div class="pacer-editor-head-actions"><button type="button" class="pacer-help-button" data-pacer-help-open="overview" aria-haspopup="dialog" aria-controls="pacer-help-dialog">ⓘ <span>Como funciona?</span></button><?php if ($editing): ?><a href="/user/pacer.php">Novo</a><?php endif; ?></div></div>
                <div class="pacer-fields">
                    ...
                    <label><span>Distância</span><div class="pacer-input-unit field-with-unit"><input type="number" name="target_distance_km" step="0.01" min="0.2" max="500" required value="<?php echo $editing ? stridebr_e(rtrim(rtrim(number_format($editing['target_distance_m'] / 1000, 3, '.', ''), '0'), '.')) : '10'; ?>"><span class="pacer-input-unit-label">km</span></div></label>
                    <fieldset class="pacer-goal is-wide"><legend>Objetivo</legend><label><input type="radio" name="goal_mode" value="target_time"<?php echo $editingGoalMode !== 'best_effort' ? ' checked' : ''; ?>><span><b>Atingir uma marca</b><small>O plano é montado para chegar próximo do tempo definido.</small></span></label><label><input type="radio" name="goal_mode" value="best_effort"<?php echo $editingGoalMode === 'best_effort' ? ' checked' : ''; ?>><span><b>Buscar o melhor tempo</b><small>O tempo serve como referência para orientar o plano.</small></span></label></fieldset>
                    ...
                    <label><span>Estratégia</span><select name="strategy" data-pacer-strategy><option value="even"<?php echo ($editing['strategy'] ?? '') === 'even' ? ' selected' : ''; ?>>Ritmo constante</option><option value="negative_split"<?php echo ($editing['strategy'] ?? 'negative_split') === 'negative_split' ? ' selected' : ''; ?>>Progressivo (negative split)</option><option value="custom"<?php echo ($editing['strategy'] ?? '') === 'custom' ? ' selected' : ''; ?>>Personalizada</option><option value="positive_split"<?php echo ($editing['strategy'] ?? '') === 'positive_split' ? ' selected' : ''; ?>>Positive split</option></select></label>
                </div>
                <details class="pacer-advanced"><summary>Opções avançadas</summary><div class="pacer-generation-fields" data-generated-options>
                    <label><span>Tolerância</span><div class="pacer-input-unit field-with-unit"><input type="number" name="tolerance_s_per_km" min="0" max="600" step="1" value="<?php echo stridebr_e((string) ($editing['default_tolerance_s_per_km'] ?? 10)); ?>"><span class="pacer-input-unit-label">s/km</span></div><small>Ex.: ±10 s/km.</small></label>
                    <label data-progressive-option><span>Progressão</span><div class="pacer-input-unit field-with-unit"><input type="number" name="progression_percent" min="0.1" max="30" step="0.5" value="<?php echo stridebr_e((string) $editingProgression); ?>"><span class="pacer-input-unit-label">%</span></div></label>
                    <label data-progressive-option><span>Tamanho do segmento</span><div class="pacer-input-unit field-with-unit"><input type="number" name="segment_distance_m" min="200" step="100" value="<?php echo stridebr_e((string) $editingSegmentDistance); ?>"><span class="pacer-input-unit-label">m</span></div></label>
                    <label data-global-hr><span>FC mínima opcional</span><div class="pacer-input-unit field-with-unit"><input type="number" name="heart_rate_floor_bpm" min="20" max="260" value="<?php echo stridebr_e((string) $editingHrFloor); ?>"><span class="pacer-input-unit-label">bpm</span></div></label>
                    <label data-global-hr><span>FC máxima opcional</span><div class="pacer-input-unit field-with-unit"><input type="number" name="heart_rate_ceiling_bpm" min="20" max="260" value="<?php echo stridebr_e((string) $editingHrCeiling); ?>"><span class="pacer-input-unit-label">bpm</span></div></label>
    ...
    <dialog class="pacer-help-dialog" id="pacer-help-dialog" data-pacer-help-dialog aria-labelledby="pacer-help-title">
        <div class="pacer-help-dialog-inner">
            <header><div><span>Guia do Pacer</span><h2 id="pacer-help-title">Como as opções afetam o plano</h2></div><button type="button" class="pacer-help-close" data-pacer-help-close aria-label="Fechar ajuda">×</button></header>
            <div class="pacer-help-content">
                <section data-pacer-help-section="goal"><h3>Objetivo</h3><dl><div><dt>Atingir uma marca</dt><dd>O plano usa o tempo definido como objetivo para a distância.</dd></div><div><dt>Buscar o melhor tempo</dt><dd>O tempo serve como referência para montar o plano e comparar seu progresso.</dd></div></dl></section>
                <section data-pacer-help-section="strategy"><h3>Estratégia</h3><dl><div><dt>Ritmo constante</dt><dd>Mantém o mesmo ritmo-alvo durante toda a distância.</dd></div><div><dt>Negative split</dt><dd>Começa mais controlado e aumenta o ritmo ao longo da distância.</dd></div><div><dt>Positive split</dt><dd>Começa mais rápido e reduz o ritmo ao longo da distância.</dd></div><div><dt>Personalizada</dt><dd>Permite definir manualmente a distância e o alvo de cada trecho.</dd></div></dl></section>
                <section data-pacer-help-section="advanced"><h3>Opções avançadas</h3><dl><div><dt>Tolerância</dt><dd>Margem de variação do ritmo antes de uma correção.</dd></div><div><dt>Progressão</dt><dd>Quanto o ritmo muda entre os trechos nas estratégias progressivas.</dd></div><div><dt>Tamanho do segmento</dt><dd>Distância usada para dividir o plano em blocos.</dd></div><div><dt>FC mínima / máxima</dt><dd>Limites opcionais configurados pelo atleta ou treinador.</dd></div><div><dt>Relógio</dt><dd>Define qual referência de tempo será usada pelo guia quando houver suporte de execução.</dd></div></dl></section>
            </div>
        </div>
    </dialog>
</main>
