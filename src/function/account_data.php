...
function accountExportData(PDO $pdo, string $userId): array
{
    ...
    $simple = [
        ...
        'temporadas' => ['SELECT t.*,m.nome AS modalidade_nome,m.slug AS modalidade_slug FROM temporadas_usuario t JOIN modalidades m ON m.idmodalidade=t.idmodalidade WHERE t.idusuario=:usuario ORDER BY t.data_inicio,t.data_criacao', true],
        'graduacoes' => ['SELECT g.*,m.nome AS modalidade_nome,m.slug AS modalidade_slug FROM graduacoes_usuario g JOIN modalidades m ON m.idmodalidade=g.idmodalidade WHERE g.idusuario=:usuario ORDER BY g.data_graduacao,g.data_criacao', true],
        'tecnicas' => ['SELECT t.*,m.nome AS modalidade_nome,m.slug AS modalidade_slug FROM tecnicas_usuario t JOIN modalidades m ON m.idmodalidade=t.idmodalidade WHERE t.idusuario=:usuario ORDER BY m.nome,t.nome_normalizado', true],
        'praticas_tecnicas' => ['SELECT p.*,t.nome AS tecnica_nome,t.idmodalidade FROM praticas_tecnica p JOIN tecnicas_usuario t ON t.idtecnica=p.idtecnica WHERE p.idusuario=:usuario ORDER BY p.data_pratica,p.data_criacao', true],
    ...
    foreach ($simple as $key => [$sql]) {
        $tableName = match ($key) {
            ...
            'temporadas' => 'temporadas_usuario',
            'graduacoes' => 'graduacoes_usuario',
            'tecnicas' => 'tecnicas_usuario',
            'praticas_tecnicas' => 'praticas_tecnica',
