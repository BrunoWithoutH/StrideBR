...
function sportPickerRenderSportRow(array $sport, string $familyLabel, string $selectedId = '', bool $favoriteControls = false): string
{
    ...
    $storedName = (string) ($sport['nome'] ?? stridebr_t('common.activity'));
    $name = stridebr_sport_name($slug, $storedName);
    $family = sportCatalogFamilyKey((string) ($sport['familia_hub'] ?? ''), (string) ($sport['categoria'] ?? ''), $slug);
    $favorite = !empty($sport['favorita']);
    $search = stridebr_lower(trim($name . ' ' . $storedName . ' ' . $familyLabel . ' ' . (string) ($sport['categoria'] ?? '')));
...
function sportPickerRenderFamilyBrowser(array $sports, string $selectedId = '', bool $favoriteControls = false): string
{
    ...
    foreach ($groups as $group) {
        ...
        $html .= '</div>';
        if ($group['more'] !== []) {
            $html .= '<button type="button" class="sport-family-more-toggle" data-sport-family-more aria-expanded="false"><span>' . stridebr_e(stridebr_t('sport_picker.more')) . '</span><span aria-hidden="true">⌄</span></button>';
...
function sportPickerRenderSelect(array $sports, array $options = []): string
{
    ...
    $placeholder = trim((string) ($options['placeholder'] ?? stridebr_t('sport_picker.choose')));
    ...
    $selectedStoredName = $selected ? (string) ($selected['nome'] ?? $placeholder) : '';
    $selectedName = $selected ? stridebr_sport_name($selectedSlug, $selectedStoredName) : ($selectedId === '' && $emptyLabel !== '' ? $emptyLabel : $placeholder);
    $attrs = '';
    ...
    foreach ($sports as $sport) {
        ...
        $html .= '<option value="' . stridebr_e($id) . '" data-slug="' . stridebr_e((string) ($sport['slug'] ?? '')) . '" data-family="' . stridebr_e($family) . '"' . $metricAttr . $routeAttr . ($selectedId === $id ? ' selected' : '') . '>' . stridebr_e(stridebr_sport_name((string) ($sport['slug'] ?? ''), (string) ($sport['nome'] ?? stridebr_t('common.activity')))) . '</option>';
    }
    $html .= '</select>';
    ...
    $html .= '<label class="sport-select-search"><span>' . stridebr_e(stridebr_t('sport_picker.search')) . '</span><input type="search" data-generic-sport-search placeholder="' . stridebr_e(stridebr_t('sport_picker.search_placeholder')) . '" autocomplete="off"></label>';
    ...
    foreach ($groups as $group) {
        ...
        $html .= '</div>';
        if ($group['more'] !== []) {
            $html .= '<button type="button" class="sport-family-more-toggle" data-generic-sport-more aria-expanded="false"><span>' . stridebr_e(stridebr_t('sport_picker.more')) . '</span><span aria-hidden="true">⌄</span></button><div class="sport-family-more-list" data-generic-sport-more-list hidden>';
            foreach ($group['more'] as $sport) $html .= sportPickerRenderGenericOption($sport, $label, $selectedId, $valueKey);
            $html .= '</div>';
        }
        $html .= '</section>';
    }
    $html .= '<div class="sport-select-no-results" data-generic-sport-no-results hidden>' . stridebr_e(stridebr_t('sport_picker.no_results')) . '</div>';
