...
$checks = [
    ...
    'progresso mostra consistência e tendências' => str_contains($progress, 'das últimas 8 semanas') && str_contains($progress, 'Comparação com as quatro semanas anteriores.') && str_contains($progress, 'Marcas pessoais'),
    ...
    'comparação de musculação separa métricas relevantes' => str_contains($compare, 'Volume calculável') && str_contains($compare, 'Maior carga') && str_contains($compare, 'Exercícios em comum'),
    'progresso e comparação aparecem na navegação' => str_contains($activitiesPage, '/user/progresso.php') && str_contains($activitiesPage, '/user/comparar-atividades.php') && str_contains($activitiesJs, '/user/comparar-atividades.php?a=') && str_contains($footer, "stridebr_t('nav.progress')") && (str_contains($footer, 'Comparar atividades') || str_contains($footer, '/user/comparar-atividades.php')) && str_contains($header, '/user/progresso.php'),
    ...
    'painel do treinador respeita permissões' => str_contains($trainer, "pode_ver_atividades") && str_contains($trainer, "pode_ver_cronograma") && str_contains($trainer, 'sem permissão') && str_contains($trainer, "if (stridebr_db_bool(\$selectedLink['pode_ver_atividades'] ?? false))"),
    'analytics tem opt-out e allowlist' => str_contains($settings, 'name="product_analytics"') && str_contains($settings, 'Não envia rota, notas, cargas nem conteúdo pessoal') && str_contains($analytics, "'activity_compared'") && str_contains($analytics, "'dashboard_customized'") && str_contains($analytics, 'if (!in_array($nome, $allowedNames, true)) return;'),
    ...
    'ações destrutivas de atividade têm chave e desfazer' => substr_count($activitiesJs, "_idempotency_key: requestKey()") >= 2 && str_contains($activitiesJs, "body.set('_idempotency_key', requestKey())") && str_contains($activitiesJs, "StrideBRUI.undo('Atividade apagada.'") && str_contains($activitiesJs, '/api/atividades-restaurar.php'),
    ...
    'legais atualizadas para a RC' => str_contains($terms, '30 de agosto de 2026') && str_contains($privacy, '1º de setembro de 2026') && str_contains($privacy, 'Analytics de produto') && str_contains($privacy, 'trechos privados') && str_contains($cookies, 'Permitir publicidade'),
    ...
    'empty states úteis existem nas áreas novas' => str_contains($progress, 'Seu progresso começa no primeiro registro') && str_contains($activitiesPage, 'Registrar atividade') && str_contains($exchangePage, 'Registre uma atividade ou importe um arquivo esportivo') && str_contains($trainer, 'Nenhum cronograma ativo.'),
];
