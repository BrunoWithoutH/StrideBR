...
$assert(str_contains($js, "let activeShareCompositionId = 'standard'") && str_contains($js, "configuration.composition === 'compact'") && str_contains($js, "const compactLayoutMode = String(compactDefinition?.layoutByFormat?.[format]") && str_contains($js, "const isWide = compactLayoutMode === 'grid'"), 'Compacto precisa ser composição adaptativa, não formato próprio.');
...
$assert(str_contains($js, 'const drawCompactShareCardSurface = async') && str_contains($js, 'const compactMetricLayout =') && str_contains($js, 'compactDefinition?.layoutByFormat?.[format]') && str_contains($js, "const layoutHeight = format === 'story' ? Math.min(height, 1350) : height"), 'Compacto precisa ter composição própria: vertical em Story/Retrato e grade em Quadrado sem esticar Story.');
...
$assert(str_contains($js, 'const metricLayout = compactMetricLayout(format, metrics.length, side, contentWidth, metricTop, metricRowHeight, compactDefinition)') && str_contains($js, "const metricRows = compactLayoutMode === 'vertical' ? metrics.length : shareMetricRowCount(metrics.length)"), 'compactos devem usar layout vertical em Story/Retrato e grid dinâmico em Quadrado.');
...
$assert(str_contains($header, '/user/biblioteca.php') && !str_contains($footer, '/user/biblioteca.php?tab=treinos') && !str_contains($footer, "stridebr_t('nav.library_exercises')"), 'Biblioteca e Exercícios devem pertencer à área de Treinos, sem voltar ao menu global mobile.');
