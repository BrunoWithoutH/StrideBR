...
return function (PDO $pdo): void {
    ...
    $phaseSchedule = cronogramaCriar($pdo, $a, 'Alpha schedule phases');
    $phaseOld = cronogramaSalvarTreino($pdo, $a, ['idcronograma' => $phaseSchedule, 'titulo' => 'Treino de fases', 'codigo' => 'B', 'dia_semana' => '4', 'hora_inicio' => '14:00', 'hora_fim' => '16:00', 'vigencia_inicio' => '2026-08-01', 'vigencia_fim' => '2026-09-10']);
    $phaseCurrent = cronogramaSalvarTreino($pdo, $a, ['idcronograma' => $phaseSchedule, 'titulo' => 'Treino de fases', 'codigo' => 'B', 'dia_semana' => '4', 'hora_inicio' => '14:00', 'hora_fim' => '16:00', 'vigencia_inicio' => '2026-09-11']);
    $sameTimeDistinct = cronogramaSalvarTreino($pdo, $a, ['idcronograma' => $phaseSchedule, 'titulo' => 'Treino distinto', 'codigo' => 'X', 'dia_semana' => '4', 'hora_inicio' => '14:00', 'hora_fim' => '15:00', 'vigencia_inicio' => '2026-09-11']);
    $allPhaseRows = cronogramaListarTreinos($pdo, $phaseSchedule, $a);
    AlphaTest::same(3, count($allPhaseRows), 'Histórico de fases foi descartado do domínio');
    $currentPhaseRows = cronogramaFiltrarTreinosVigentes($allPhaseRows, '2026-09-19');
    $currentPhaseIds = array_map(static fn(array $row): string => (string) $row['idtreino'], $currentPhaseRows);
    AlphaTest::assert(!in_array($phaseOld, $currentPhaseIds, true), 'Fase histórica apareceu como rotina atual');
    AlphaTest::assert(in_array($phaseCurrent, $currentPhaseIds, true), 'Fase vigente desapareceu da rotina atual');
    AlphaTest::assert(in_array($sameTimeDistinct, $currentPhaseIds, true), 'Treino genuinamente distinto no mesmo horário foi deduplicado');
    $futurePhaseRows = cronogramaFiltrarTreinosVigentes($allPhaseRows, '2026-09-05');
    $futurePhaseIds = array_map(static fn(array $row): string => (string) $row['idtreino'], $futurePhaseRows);
    AlphaTest::assert(in_array($phaseOld, $futurePhaseIds, true) && !in_array($phaseCurrent, $futurePhaseIds, true), 'Filtro de vigência não respeitou a fase histórica de referência');
