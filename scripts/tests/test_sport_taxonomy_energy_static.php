...
$assert(str_contains($presenter, "'energia' => \$energy") && str_contains($presenter, "'rotulo' => stridebr_t('activity.calories')") && !str_contains($progress, "'calories' => true"), 'calorias precisam continuar no detalhe/compartilhamento sem virar KPI obrigatório de Progresso.');
...
$assert(($cardioDashboard['summary']['run']['current']['activities'] ?? 0) === 1 && ($cardioDashboard['summary']['run']['current']['avg_pace_km_s'] ?? 0) > 0 && isset($cardioDashboard['active']['run']), 'Cardio precisa preservar subáreas e métricas próprias na camada analítica.');
...
$assert(str_contains((string) file_get_contents($root . '/src/function/sport_hub.php'), 'function sportHubAthleticsDashboard') && str_contains($progress, 'progress-small-multiples') && str_contains($progress, 'progress-sport-list'), 'Progresso precisa manter a base analítica específica sem despejá-la toda no dashboard padrão.');
...
$assert(str_contains((string) file_get_contents($root . '/src/function/sport_hub.php'), 'function sportHubSessionDashboard') && str_contains($progress, 'sportHubModalitiesBreakdown($currentActivities)'), 'hubs de sessão precisam preservar métricas próprias enquanto o Progresso padrão usa uma visão comparável por modalidade.');
