...
$assert(str_contains((string) file_get_contents($root . '/src/function/sport_hub.php'), 'function sportHubAthleticsDashboard') && str_contains($progress, 'sportHubPeriodSeries($currentActivities') && str_contains($progress, 'progress-sport-list'), 'Progresso precisa manter renderers específicos e uma lista legível de esportes sem voltar ao dashboard genérico antigo.');
...
$assert(str_contains((string) file_get_contents($root . '/src/function/sport_hub.php'), 'function sportHubSessionDashboard') && str_contains($progress, 'sportHubSportsPeriodSummary($availableSports, $navigationPeriodActivities)'), 'hubs de sessão precisam preservar métricas próprias enquanto a visão geral usa inventário ativo mais histórico.');
