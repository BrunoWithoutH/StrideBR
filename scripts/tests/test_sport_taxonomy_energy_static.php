...
$activityPanelShared = (string) file_get_contents($root . '/src/layout/activity_model_panel_shared.php');
...
$assert(str_contains($energyCode, 'atividadeEnergiaPesoParaData') && str_contains($settings, "stridebr_t('settings.weight_history')") && str_contains($settings, 'historico_peso_usuario'), 'peso histórico precisa ser usado e editável.');
$assert(str_contains($presenter, "'energia' => \$energy") && str_contains($presenter, "'rotulo' => stridebr_t('activity.calories')") && str_contains($progress, 'calories_kcal'), 'calorias precisam voltar para detalhe, compartilhamento e progresso.');
...
$assert(str_contains($activityPanelShared, "stridebr_t('activity.attempts')") && str_contains($progress, "stridebr_t('progress.best_marks')") && str_contains($progress, "stridebr_t('progress.best_times')") && str_contains((string) file_get_contents($root . '/src/function/sport_hub.php'), 'sportHubAthleticsDashboard'), 'atletismo precisa ter UX de tentativas e painel próprio de evolução.');
...
$assert(($cardioDashboard['summary']['run']['current']['activities'] ?? 0) === 1 && ($cardioDashboard['summary']['run']['current']['avg_pace_km_s'] ?? 0) > 0 && isset($cardioDashboard['active']['run']) && str_contains($progress, "stridebr_t('progress.last_8_weeks')"), 'Cardio precisa ter subáreas e métricas próprias por modalidade.');
...
$assert(str_contains($progress, "stridebr_t('progress.effort_technique')") && str_contains($progress, "stridebr_t('progress.recent_sessions')") && str_contains($progress, "stridebr_t('progress.new_record')") && str_contains($progress, "stridebr_t('progress.best_legal_wind_mark'"), 'hub precisa apresentar técnica, evolução de força e validade de vento no atletismo.');
...
$assert(str_contains($progress, "stridebr_t('progress.recent_sessions')") && str_contains($progress, "stridebr_t('progress.win_rate')") && str_contains($progress, "stridebr_t('progress.best_score')"), 'hubs de raquetes, equipe, lutas e precisão precisam mostrar métricas próprias.');
