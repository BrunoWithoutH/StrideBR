...
$checks = [
    ...
    'detalhe distingue GPS Web de GPS do app sem copy de navegador no Android' => str_contains($presenter, "'client_source' =>") && str_contains($presenter, "'stridebr_android' ? 'app' : 'web'") && str_contains($activitiesJs, "tr(appGps ? 'activity.gps.source_app' : 'activity.gps.source_web')") && str_contains($activitiesJs, "tr(appGps ? 'activity.gps.gps_estimate' : 'activity.gps.browser_estimate')") && str_contains($activitiesJs, "appGps ? trn('activity.gps.capture_gaps.one'") && str_contains($activitiesCss, '.activity-gps-notice'),
