...
$assert(str_contains($activities, '@media (min-width:901px)') && str_contains($activities, 'overscroll-behavior:auto;') && str_contains($activities, 'html.activity-detail-open,html.activity-detail-open body{overflow:auto}'), 'Preview desktop precisa encadear wheel com a página e não bloquear body.');
$assert(str_contains($activities, '@media (max-width:900px)') && str_contains($activities, '.activity-detail-drawer{z-index:var(--z-drawer)}'), 'Mobile precisa preservar contrato de drawer.');
