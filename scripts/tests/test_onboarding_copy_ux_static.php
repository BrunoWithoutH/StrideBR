...
$checks = [
    'onboarding mantém área e ações estáveis' => str_contains($uiCss, '.signup-onboarding-card{') && str_contains($uiCss, 'height:min(720px,calc(100dvh - 88px))') && str_contains($uiCss, '.signup-onboarding-actions') && str_contains($uiCss, 'border-top:1px solid var(--ui-border-soft)'),
    ...
    'lista de esportes tem busca grupos e ícones do site' => str_contains($signup, 'data-signup-sport-search') && str_contains($signup, 'signupSportGroups') && str_contains($signup, "stridebr_t('onboarding.common')") && str_contains($signup, 'data-signup-sport-more') && str_contains($signup, 'stridebr_sport_icon_html'),
    'objetivos e acompanhamento têm opções orientadas a uso' => str_contains($signup, "stridebr_t('onboarding.goal.routine')") && str_contains($signup, "stridebr_t('onboarding.tracking.distance')") && str_contains($signup, "stridebr_t('onboarding.tracking.strength_help')"),
    ...
    'progresso usa filtros próprios sem reabrir onboarding' => str_contains($progress, 'class="progress-filterbar"') && str_contains($progress, 'name="sport"') && str_contains($progress, 'name="metric"') && !str_contains($progress, '/user/onboarding.php'),
