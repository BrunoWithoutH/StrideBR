...
$menuJs = $read('public/assets/js/scripts.js');
$assert(str_contains($css, '.user-menu-content.is-height-compact') && str_contains($css, '.user-menu-content.is-height-scroll{overflow-y:auto}') && str_contains($css, 'flex-direction:row') && str_contains($menuJs, 'visualViewport?.height') && str_contains($menuJs, 'menu.scrollHeight > available'), 'menu adapts to measured height and keeps scroll fallback');
