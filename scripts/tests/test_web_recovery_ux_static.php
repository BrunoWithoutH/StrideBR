...
$assert(!str_contains($activitiesPage, 'data-summary-elevation') && !str_contains($detailJs, 'activity.elevation_profile') && !str_contains($detailJs, 'activity.route_gain'), 'Histórico e Activity Detail não podem expor elevação.');
...
$assert(!str_contains($routesPage, '+<?php echo (int)round((float)$route[\'elevation_gain_m\'])') && !str_contains($progressPage, 'summary.elevation') && !str_contains($exchangeJs, 'activity.summary.elevation'), 'Rotas, progresso e importação não podem apresentar elevação.');
...
$assert(str_contains($comparisonPage, "preg_match('/eleva|desn[ií]vel|relevo|altitude|ganho/i'") && str_contains($activitiesJs, 'isElevationPresentation'), 'Comparação e presenter Web precisam filtrar métricas de elevação já presentes nos dados.');
