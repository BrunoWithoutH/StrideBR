...
$assert(str_contains($domain, "if (function_exists('stridebr_generate_id')) return stridebr_generate_id();") && !str_contains($domain, "if (function_exists('stridebr_generate_id')) return notificacaoGerarId();"), 'Gerador de ID de notificação precisa delegar sem auto-recursão.');
