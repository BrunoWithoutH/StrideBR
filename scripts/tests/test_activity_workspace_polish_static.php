...
$assert(str_contains($js, "const isWide = compactLayoutMode === 'grid'") && str_contains($js, "const layoutHeight = format === 'story' ? Math.min(height, 1350) : height"), 'Compacto deve adaptar para grade no Quadrado e vertical centralizado no Story.');
...
$assert(!str_contains($js, 'shareRoundedRect(context, x, y, tileWidth, tileHeight, tileRadius)') && str_contains($js, 'const metricLayout = compactMetricLayout(format, metrics.length, side, contentWidth, metricTop, metricRowHeight, compactDefinition)') && str_contains($js, 'const metricRowHeight = isWide ? 194 : 174'), 'compactos precisam usar a composição dinâmica de métricas sem tiles.');
$assert(str_contains($js, "supportsMap: false") && str_contains($js, "definition.supportsMap && hasRoute") && str_contains($js, "showMapBase: Boolean(content === 'route'") && str_contains($js, "definition.supportsMap && hasRoute"), 'mapa-base deve depender da composição/rota e desaparecer no Compacto.');
...
$assert(str_contains($js, "mode: detailExpanded ? 'detail' : 'preview'") && str_contains($js, 'data-open-full-activity-details') && !str_contains(substr($js, strpos($js, 'const renderDetail ='), strpos($js, 'const setDetailExpanded =') - strpos($js, 'const renderDetail =')), 'drawElevationProfile(elevationSvg'), 'prévia rápida precisa usar modo compacto, abrir detalhe sob demanda e não renderizar perfil de elevação.');
...
$assert(str_contains($page, 'class="activity-toolbar-tools"') && str_contains($page, '/user/importar-exportar.php') && str_contains($page, 'data-detail-compare'), 'Atividades precisa manter ferramentas secundárias agrupadas e Comparar contextual no detalhe.');
