...
$assert(str_contains($page, 'role="dialog" aria-modal="false"') && str_contains($activityJs, "detailPanel?.setAttribute('aria-modal', shouldBeModal ? 'true' : 'false')"), 'Preview desktop deve ser não modal e mobile/expanded devem alternar aria-modal dinamicamente.');
...
$assert(str_contains($css, '.activity-detail-drawer.is-expanded-detail .activity-detail-panel{') && str_contains($css, 'overflow-y:auto'), 'Detalhe expandido deve preservar scroll interno próprio.');
