...
$assert(str_contains($progress, 'sportHubActivitiesInWindow($filteredActivities') && str_contains($progress, 'sportHubSummaryMetrics($currentActivities)') && str_contains($progress, 'sportHubPeriodSeries($currentActivities') && str_contains($progress, 'sportHubConsistencySummary'), 'resumos, séries adaptativas e consistência precisam respeitar período e modalidade selecionados.');
