...
with sync_playwright() as p:
    ...
    page=browser.new_page(viewport={'width':900,'height':600});boot(page,'pt-BR')
    hours=page.locator('[data-primary-unit] [data-duration-hours]')
    minutes=page.locator('[data-primary-unit] [data-duration-minutes]')
    seconds=page.locator('[data-primary-unit] [data-duration-seconds]')
    hours.fill(''); hours.type('01',delay=15); page.wait_for_timeout(15)
    check(page.evaluate('document.activeElement?.hasAttribute("data-duration-minutes")') is True,'2 dígitos de horas avançam para minutos')
    minutes.fill(''); minutes.type('18',delay=15); page.wait_for_timeout(15)
    check(page.evaluate('document.activeElement?.hasAttribute("data-duration-seconds")') is True,'2 dígitos de minutos avançam para segundos')
    seconds.fill(''); seconds.type('02',delay=15); page.wait_for_timeout(20)
    check(page.evaluate('document.activeElement?.hasAttribute("data-duration-seconds")') is False,'segundos encerram o fluxo quando ms está desligado')
    check(page.locator('[data-primary-unit] [data-duration-value]').input_value()=='01:18:02','entrada 2/2/2 mantém valor canônico')
    minutes.focus(); minutes.fill(''); minutes.press('Backspace'); page.wait_for_timeout(10)
    check(page.evaluate('document.activeElement?.hasAttribute("data-duration-hours")') is True,'Backspace em minutos vazio volta para horas')
    hours.focus(); page.evaluate("navigator.clipboard?.writeText?.('02:03:04')");
    page.locator('[data-primary-unit] [data-duration-hours]').evaluate("el=>{const dt=new DataTransfer();dt.setData('text/plain','02:03:04');el.dispatchEvent(new ClipboardEvent('paste',{clipboardData:dt,bubbles:true,cancelable:true}))}")
    page.wait_for_timeout(15)
    check(page.locator('[data-primary-unit] [data-duration-value]').input_value()=='02:03:04','paste de duração completa continua usando o fluxo existente')
    page.locator('[data-duration-precision-toggle]').click(); page.wait_for_timeout(10)
    seconds=page.locator('[data-primary-unit] [data-duration-seconds]'); milliseconds=page.locator('[data-primary-unit] [data-duration-milliseconds]')
    seconds.fill(''); seconds.type('05',delay=15); page.wait_for_timeout(15)
    check(page.evaluate('document.activeElement?.hasAttribute("data-duration-milliseconds")') is True,'segundos avançam para ms quando precisão está ativa')
    milliseconds.fill(''); milliseconds.type('438',delay=15); milliseconds.blur(); page.wait_for_timeout(20)
    check(milliseconds.input_value()=='438' and page.locator('[data-primary-unit] [data-duration-value]').input_value().endswith('05.438'),'milissegundos usam 3 dígitos')
    segment_h=page.locator('[data-segment] [data-duration-hours]'); segment_m=page.locator('[data-segment] [data-duration-minutes]')
    segment_h.fill(''); segment_h.type('03',delay=15); page.wait_for_timeout(10)
    check(page.evaluate('document.activeElement?.hasAttribute("data-duration-minutes")') is True,'Trechos usam o mesmo auto-avanço 2/2/2/3')
    segment_m.fill(''); segment_m.press('Backspace'); page.wait_for_timeout(10)
    check(page.evaluate('document.activeElement?.hasAttribute("data-duration-hours")') is True,'Trechos usam Backspace para campo anterior')
    page.close()
    browser.close();print(f'✓ activity duration ms UI: {count[0]} assertions; PT/EN + auto-advance 2/2/2/3')
